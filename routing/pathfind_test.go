...
// all edges.
func noProbabilitySource(route.Vertex, route.Vertex, lnwire.MilliAtom) float64 {
	return 1
}
...
func parseTestGraph(path string) (*testGraphInstance, error) {
	...
	for _, node := range g.Nodes {
		...
		// If the alias is unique, then add the node to the alias map
		// for easy lookup.
		...
		if len(privBytes) > 0 {
			key := secp256k1.PrivKeyFromBytes(
				privBytes,
			)
			derivedPub := key.PubKey()
...
func symmetricTestChannel(alias1, alias2 string, capacity dcrutil.Amount,
...
func asymmetricTestChannel(alias1, alias2 string, capacity dcrutil.Amount,
...
func createTestGraphFromChannels(testChannels []*testChannel, source string) (
	*testGraphInstance, error) {
	...
	for _, testChannel := range testChannels {
		...
		capacity := lnwire.MilliAtom(testChannel.Capacity * 1000)
		...
		edgeInfo := channeldb.ChannelEdgeInfo{
			...
			DecredKey1Bytes: node1Vertex,
			NodeKey2Bytes:   node2Vertex,
			DecredKey2Bytes: node2Vertex,
		}
		...
		if node1.testChannelPolicy != nil {
			...
			edgePolicy := &channeldb.ChannelEdgePolicy{
				...
				FeeBaseMAtoms:             node1.FeeBaseMAtoms,
		...
		if node2.testChannelPolicy != nil {
			...
			edgePolicy := &channeldb.ChannelEdgePolicy{
				...
				FeeBaseMAtoms:             node2.FeeBaseMAtoms,
...
// TestNewRoute tests whether the construction of hop payloads by newRoute is
// executed correctly.
...
func TestNewRoutePathTooLong(t *testing.T) {
	...
	payAmt := lnwire.MilliAtom(100001)
...
func testProbabilityRouting(t *testing.T, paymentAmt dcrutil.Amount,
	...
	testChannels := []*testChannel{
		...
		symmetricTestChannel("a1", "a2", 100000, &testChannelPolicy{
			Expiry:        144,
			FeeBaseMAtoms: lnwire.NewMAtomsFromAtoms(5),
			MinHTLC:       1,
		}, 10),
		symmetricTestChannel("a2", "target", 100000, &testChannelPolicy{
			Expiry:        144,
			FeeBaseMAtoms: lnwire.NewMAtomsFromAtoms(8),
			MinHTLC:       1,
		}, 11),
		symmetricTestChannel("b", "target", 100000, &testChannelPolicy{
			Expiry:        100,
			FeeBaseMAtoms: lnwire.NewMAtomsFromAtoms(25),
			MinHTLC:       1,
		}, 20),
	}
	...
	ctx.pathFindingConfig = PathFindingConfig{
		AttemptCost:    lnwire.NewMAtomsFromAtoms(9),
		AttemptCostPPM: 10000,
		MinProbability: minProbability,
	}

	path, err := ctx.findPath(
		target, lnwire.NewMAtomsFromAtoms(paymentAmt),
	)
	if expectedChan == 0 {
		if err != errNoPathFound {
...
func TestEqualCostRouteSelection(t *testing.T) {
	...
	testChannels := []*testChannel{
		...
		symmetricTestChannel("a", "target", 100000, &testChannelPolicy{
			Expiry:        144,
			FeeBaseMAtoms: lnwire.NewMAtomsFromAtoms(6),
			MinHTLC:       1,
		}, 1),
		symmetricTestChannel("b", "target", 100000, &testChannelPolicy{
			Expiry:        100,
...
func (c *pathFindingTestContext) findPath(target route.Vertex,
	amt lnwire.MilliAtom) ([]*channeldb.CachedEdgePolicy,
	error) {

...
func createTestRoute(amt lnwire.MilliAtom,
	aliasMap map[string]route.Vertex) (*route.Route, error) {

	hopFee := lnwire.NewMAtomsFromAtoms(3)
	hop1 := aliasMap["b"]
	hop2 := aliasMap["c"]
	hops := []*route.Hop{
		{
...
func testPaymentLifecycle(t *testing.T, test paymentLifecycleTestCase,
	paymentAmt lnwire.MilliAtom, startingBlockHeight uint32,
...
func TestPaymentState(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			name:                      "state 100",
			numShardsInFlight:         1,
			remainingAmt:              lnwire.MilliAtom(0),
			terminate:                 false,
...

		{
			...
			name:                      "state 110",
			numShardsInFlight:         1,
			remainingAmt:              lnwire.MilliAtom(1),
			terminate:                 false,
			expectedTerminated:        false,
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			pl := &paymentLifecycle{
				...
				totalAmount: lnwire.MilliAtom(tc.totalAmt),
				feeLimit:    lnwire.MilliAtom(tc.feeLimit),
			}
...
func makeAttemptInfo(total, amtForwarded int) channeldb.HTLCAttemptInfo {
	hop := &route.Hop{AmtToForward: lnwire.MilliAtom(amtForwarded)}
	return channeldb.HTLCAttemptInfo{
		Route: route.Route{
			TotalAmount: lnwire.MilliAtom(total),
			Hops:        []*route.Hop{hop},
		},
	}
}

...
// TestFindRoutesWithFeeLimit asserts that routes found by the FindRoutes
// method within the channel router contain a total fee less than or equal to
// the fee limit.
...
func TestChannelUpdateValidation(t *testing.T) {
	...
	ctx.router.cfg.Payer.(*mockPaymentAttemptDispatcher).setPaymentResult(
		func(firstHop lnwire.ShortChannelID) ([32]byte, error) {
			...
			v := ctx.aliases["b"]
			source, err := secp256k1.ParsePubKey(v[:])
			if err != nil {
				t.Fatal(err)
			}
	...
	ctx.router.cfg.Payer.(*mockPaymentAttemptDispatcher).setPaymentResult(
		func(firstHop lnwire.ShortChannelID) ([32]byte, error) {
			...
			if firstHop == roasbeefSongoku {
				sourceKey, err := secp256k1.ParsePubKey(
					sourceNode[:],
				)
				if err != nil {
					t.Fatal(err)
...
func TestRouterPaymentStateMachine(t *testing.T) {
	...
	// requests one.
	rt, err := route.NewRouteFromHops(
		lnwire.MilliAtom(10000), 100, testGraph.aliasMap["a"], hops,
	)
	if err != nil {
		t.Fatalf("unable to create route: %v", err)
	}
	...
	for _, test := range tests {
		...
		errSource, err := secp256k1.ParsePubKey(hop1[:])
		if err != nil {
			t.Fatalf("unable to fetch source node pub: %v", err)
		}

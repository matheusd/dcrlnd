...
type testCtx struct {
	...
	privKeys map[string]*secp256k1.PrivateKey
...
func createTestCtxFromGraphInstanceAssumeValid(t *testing.T,
	...
	sessionSource := &SessionSource{
		Graph: cachedGraph,
		QueryBandwidth: func(
			c *channeldb.DirectedChannel) lnwire.MilliAtom {

			return lnwire.NewMAtomsFromAtoms(c.Capacity)
		},
		PathFindingConfig: pathFindingConfig,
		MissionControl:    mc,
	}
	...
	router, err := New(Config{
		...
		GraphPruneInterval: time.Hour * 2,
		QueryBandwidth: func(
			e *channeldb.DirectedChannel) lnwire.MilliAtom {
...
func TestSendPaymentErrorFeeInsufficientPrivateEdge(t *testing.T) {
	...
	var (
		...
		feeBaseMAtoms    = uint32(15)
		expiryDelta      = uint16(32)
		sgNode           = ctx.aliases["songoku"]
	)

	sgNodeID, err := secp256k1.ParsePubKey(sgNode[:])
	require.NoError(t, err)
	...
	// original fee.
	updatedFeeBaseMAtoms := feeBaseMAtoms * 2
	errChanUpdate := lnwire.ChannelUpdate{
		ShortChannelID: lnwire.NewShortChanIDFromInt(privateChannelID),
		Timestamp:      uint32(testTime.Add(time.Minute).Unix()),
		BaseFee:        updatedFeeBaseMAtoms,
	...
	require.Equal(t,
		lnwire.MilliAtom(updatedFeeBaseMAtoms).String(),
...
func TestSendPaymentPrivateEdgeUpdateFeeExceedsLimit(t *testing.T) {
	...
	var (
		...
		expiryDelta      = uint16(32)
		sgNode           = ctx.aliases["songoku"]
		feeLimit         = lnwire.MilliAtom(500000)
	)
	...
	updatedFeeBaseMAtoms := feeBaseMSat + uint32(feeLimit)
	errChanUpdate := lnwire.ChannelUpdate{
		ShortChannelID: lnwire.NewShortChanIDFromInt(privateChannelID),
		Timestamp:      uint32(testTime.Add(time.Minute).Unix()),
		BaseFee:        updatedFeeBaseMAtoms,
...
func TestAddEdgeUnknownVertexes(t *testing.T) {
	...
	connectNodeKey, err := secp256k1.ParsePubKey(connectNode[:])
	if err != nil {
		t.Fatal(err)
	}
	...
	edge.DecredKey2Bytes = node2Bytes
...
func TestUnknownErrorSource(t *testing.T) {
	...
	payment := LightningPayment{
		...
		Amount:      lnwire.NewMAtomsFromAtoms(1000),
		FeeLimit:    noFeeLimit,
		paymentHash: &payHash,
	}
...
func TestSendToRouteStructuredError(t *testing.T) {
	...
	// in a call to SendToRoute. SendToRoute also applies channel updates,
	// but it saves us from including RequestRoute in the test scope too.
	const payAmt = lnwire.MilliAtom(10000)
	hop1 := ctx.aliases["b"]
	hop2 := ctx.aliases["c"]
	hops := []*route.Hop{
		{
...
func TestSendToRouteMultiShardSend(t *testing.T) {
	...
	const numShards = 3
	const payAmt = lnwire.MilliAtom(numShards * 10000)
...
func TestBuildRoute(t *testing.T) {
	...
	testChannels := []*testChannel{
		...
		symmetricTestChannel("a", "b", chanCapSat/2, &testChannelPolicy{
			...
			FeeRate: 20000,
			MinHTLC: lnwire.NewMAtomsFromAtoms(5),
			MaxHTLC: lnwire.NewMAtomsFromAtoms(chanCapSat / 2),
		}, 6),
	...
	amt := lnwire.NewMAtomsFromAtoms(100)
...
func createDummyLightningPayment(t *testing.T,
	target route.Vertex, amt lnwire.MilliAtom) *LightningPayment {
...
func TestSendMPPaymentSucceed(t *testing.T) {
	...
	// resumePayment.
	paymentAmt := lnwire.MilliAtom(10000)
...
func TestSendMPPaymentSucceedOnExtraShards(t *testing.T) {
	...
	// Mock the methods to the point where we are inside the function
	// resumePayment.
	paymentAmt := lnwire.MilliAtom(20000)
	req := createDummyLightningPayment(
		t, testGraph.aliasMap["c"], paymentAmt,
	)
	identifier := lntypes.Hash(req.Identifier())

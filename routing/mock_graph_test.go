...
// capacities.
// nolint:unparam
func (m *mockGraph) addChannel(node1, node2 *mockNode, capacity dcrutil.Amount) {
	id := m.nextChanID
	m.nextChanID++
	...
	m.nodes[node1.pubkey].channels[node2.pubkey] = &mockChannel{
		capacity: capacity,
		id:       id,
		balance:  lnwire.NewMAtomsFromAtoms(capacity / 2),
	}
	...
	for peer, channel := range node.channels {
		...
		err := cb(
			...
			&channeldb.ChannelEdgePolicy{
				...
				FeeBaseMAtoms: peerNode.baseFee,
			},
		)
		if err != nil {
			return err

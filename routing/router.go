...
import (
	...
	"github.com/decred/dcrlnd/routing/shards"
...
type Config struct {
	...
	// returned.
	QueryBandwidth func(*channeldb.DirectedChannel) lnwire.MilliAtom
...
func (r *ChannelRouter) networkHandler() {
	...
	//
	// See https://github.com/decred/dcrlnd/issues/4892.
	...
	for {
		...
		case chainUpdate, ok := <-r.newBlocks:
			...
			currentHeight := int64(atomic.LoadUint32(&r.bestHeight))
			switch {
			...
			case chainUpdate.Height > currentHeight+1:
				...
				err := r.getMissingBlocks(uint32(currentHeight), chainUpdate)
				if err != nil {
...
func (r *ChannelRouter) getMissingBlocks(currentHeight uint32,
	...
	blockDifference := int(chainUpdate.Height - int64(currentHeight))
...
func (r *ChannelRouter) processUpdate(msg interface{},
	...
	case *channeldb.ChannelEdgeInfo:
		...
		if err != nil {
			...
			//
			// * btcd:
			//    * https://github.com/decred/dcrd/blob/master/rpcserver.go#L1316
			//    * https://github.com/decred/dcrd/blob/master/rpcserver.go#L1086
			// * bitcoind:
		...
		fundingPkScript, err := input.ScriptHashPkScript(witnessScript)
		if err != nil {
			return err
		}
...
func (r *ChannelRouter) sendPayment(
	totalAmt, feeLimit lnwire.MilliAtom, identifier lntypes.Hash,
...
// applyChannelUpdate validates a channel update and if valid, applies it to
// the database. It returns a bool indicating whether the updates was
// successful.
...
func generateBandwidthHints(sourceNode route.Vertex, graph *channeldb.ChannelGraph,
	queryBandwidth func(*channeldb.DirectedChannel) lnwire.MilliAtom) (
	map[uint64]lnwire.MilliAtom, error) {
...
func (r *ChannelRouter) BuildRoute(amt *lnwire.MilliAtom,
	...
	var runningAmt lnwire.MilliAtom
	if useMinAmt {

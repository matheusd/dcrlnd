...
var (
	// defaultDcrlncliDir is the default directory to store the profile file
	// in. This defaults to:
	//   C:\Users\<username>\AppData\Local\Dcrlncli\ on Windows
	//   ~/.dcrlncli/ on Linux
	//   ~/Library/Application Support/Dcrlncli/ on MacOS
	defaultDcrlncliDir = dcrutil.AppDataDir("dcrlncli", false)

	// defaultProfileFile is the full, absolute path of the profile file.
	defaultProfileFile = path.Join(defaultDcrlncliDir, "profiles.json")
)

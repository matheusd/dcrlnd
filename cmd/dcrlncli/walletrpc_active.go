...
var bumpCloseFeeCommand = cli.Command{
	...
	Flags: []cli.Flag{
		...
		cli.Uint64Flag{
			Name: "atoms_per_byte",
			Usage: "a manual fee expressed in atoms/byte that " +
				"should be used when sweeping the output",
		},
	},
...
func fundPsbt(ctx *cli.Context) error {
	...
	// Parse fee flags.
	switch {
	case ctx.IsSet("conf_target") && ctx.IsSet("atoms_per_byte"):
...
var listAccountsCommand = cli.Command{
	...
	Description: `
	Retrieves all accounts belonging to the wallet by default. A name
	filter can be provided to filter through all of the wallet

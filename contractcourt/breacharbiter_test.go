...
func TestBreachCreateJusticeTx(t *testing.T) {
	...
	aliceKeyPriv := secp256k1.PrivKeyFromBytes(
...
func getSpendTransactions(signer input.Signer, chanPoint *wire.OutPoint,
	...
	for i := range retInfo.breachedOutputs {
		...
		case input.HtlcOfferedRevoke:
			...
			htlcSweep.TxIn[0].SignatureScript, err = input.WitnessStackToSigScript(inputScript.Witness)
			if err != nil {
				return nil, err
			}
		}
...
// createInitChannels creates two initialized test channels funded with 10 DCR,

...
func (b *BreachArbiter) sweepSpendableOutputsTxn(txSize int64,
	...
	for _, input := range inputs {
		totalAmt += dcrutil.Amount(input.SignDesc().Output.Value)
	}

	// We'll actually attempt to target inclusion within the next two
	// blocks as we'd like to sweep these funds back into our wallet ASAP.
	feePerKB, err := b.cfg.Estimator.EstimateFeePerKB(justiceTxConfTarget)
	if err != nil {
		return nil, err
	}
	...
	if err := standalone.CheckTransactionSanity(txn, uint64(b.cfg.NetParams.MaxTxSize)); err != nil {
	...
	addWitness := func(idx int, so input.Input) error {
		...
		txn.TxIn[idx].SignatureScript, err = input.WitnessStackToSigScript(inputScript.Witness)
...
// backed by a bboltdb bucket. The primary responsibility of the retribution

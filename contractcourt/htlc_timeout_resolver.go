...
func (h *htlcTimeoutResolver) chainDetailsToWatch() (*wire.OutPoint, []byte, error) {
	...
	sigScriptPushes, err := input.SigScriptToWitnessStack(sigScript)
	if err != nil {
		return nil, nil, err
	}
...
func isSuccessSpend(spend *chainntnfs.SpendDetail, localCommit bool) bool {
	...
	spendingSigScript := spendingInput.SignatureScript
	spendingWitness, err := input.SigScriptToWitnessStack(spendingSigScript)
	if err != nil {
		log.Errorf("Error decoding pushed data in sigScript: %v", err)
		return false
	}
	...
	// outgoing HTLCs are:
	//
	//  RECVR: <sender sig> <recvr sig> <preimage> (2nd level success spend)
	//  REVOK: <sig> <key>
	//  SENDR: <sig> 0
	//
	...
	// outgoing HTLC are:
	//
	//  SENDR: <sendr sig>  <recvr sig> <0> (2nd level timeout)

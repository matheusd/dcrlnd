...
func addFakeHTLC(t *testing.T, htlcAmount lnwire.MilliAtom, id uint64,
...
func TestChainWatcherDataLossProtect(t *testing.T) {
	...
	dlpScenario := func(testCase dlpTestCase) bool {
		...
		case uniClose = <-chanEvents.RemoteUnilateralClosure:
			...
			if uniClose.RemoteCommit.FeePerKB != blankCommit.FeePerKB {
	...
	err := quick.Check(dlpScenario, &quick.Config{
		...
		Values: func(v []reflect.Value, rand *rand.Rand) {
			...
			// We need to ensure it will never exceed 1/2 the max
			// number of HTLCs, otherwise adding so many HTLCs will
			// fail.
			for stateNum > input.MaxHTLCNumber/2 {
				stateNum = uint8(rand.Int31())
			}
			...
			broadcastNum := stateNum + 1 + uint8(broadcastRange)

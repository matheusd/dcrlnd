...
func testHeightExpiryWithRegistry(t *testing.T, numParts int, settle bool) {
	...
	htlcAmt := invoice.Terms.Value / lnwire.MilliAtom(numParts)
...
func testSpontaneousAmpPayment(
	...
	const (
		totalAmt = lnwire.MilliAtom(360)
		expiry   = uint32(testCurrentHeight + 20)
	)

	var (
		shardAmt = totalAmt / lnwire.MilliAtom(numShards)
		payAddr  [32]byte
		setID    [32]byte
	)

...
for i in $SYS; do
    ...
    env CGO_ENABLED=0 GOOS=$OS GOARCH=$ARCH GOARM=$ARM go build -v -trimpath -ldflags "-s -w -buildid= $COMMITFLAGS" -tags="invoicesrpc walletrpc watchtowerrpc" github.com/decred/dcrlnd/cmd/dcrlncli
    cd ..

...
func testSingleConfirmationNotification(miner *rpctest.Harness, vw *rpctest.VotingWallet,
	...
	// We'd like to test the case of being notified once a txid reaches a
	// *single* confirmation.
	//
	...
	_, currentHeight, err := miner.Node.GetBestBlock(ctxb)
	if err != nil {
	...
	blockHash, err := vw.GenerateBlocks(ctxb, 1)
	if err != nil {
	...
	case confInfo := <-confIntent.Confirmed:
		...
		msgBlock, err := miner.Node.GetBlock(ctxb, blockHash[0])
		if err != nil {
...
func testMultiConfirmationNotification(miner *rpctest.Harness, vw *rpctest.VotingWallet,
	notifier chainntnfs.TestChainNotifier, scriptDispatch bool, t *testing.T) {

	// We'd like to test the case of being notified once a txid reaches N
	// confirmations, where N > 1.
	//
	...
	if _, err := vw.GenerateBlocks(ctxb, 6); err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testBatchConfirmationNotification(miner *rpctest.Harness, vw *rpctest.VotingWallet,
	notifier chainntnfs.TestChainNotifier, scriptDispatch bool, t *testing.T) {

	// We'd like to test a case of serving notifications to multiple
	// clients, each requesting to be notified once a txid receives various
	...
	for i, numConfs := range confSpread {
		...
		if _, err := vw.GenerateBlocks(ctxb, blocksToGen); err != nil {
			t.Fatalf("unable to generate single block: %v", err)
		}
...
func testSpendNotification(miner *rpctest.Harness, vw *rpctest.VotingWallet,
	notifier chainntnfs.TestChainNotifier, scriptDispatch bool, t *testing.T) {
	...
	outpoint, output, privKey := chainntnfs.CreateSpendableOutput(t, miner, vw)
	...
	spenderHash, err := miner.Node.SendRawTransaction(ctxb, spendingTx, true)
	if err != nil {
	...
	if _, err := vw.GenerateBlocks(ctxb, 1); err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testBlockEpochNotification(miner *rpctest.Harness, vw *rpctest.VotingWallet,
	...
	if _, err := vw.GenerateBlocks(ctxb, numBlocks); err != nil {
...
func testMultiClientConfirmationNotification(miner *rpctest.Harness, vw *rpctest.VotingWallet,
	notifier chainntnfs.TestChainNotifier, scriptDispatch bool, t *testing.T) {
...
func testTxConfirmedBeforeNtfnRegistration(miner *rpctest.Harness, vw *rpctest.VotingWallet,
	notifier chainntnfs.TestChainNotifier, scriptDispatch bool, t *testing.T) {
	...
	_, err = vw.GenerateBlocks(ctxb, 1)
	if err != nil {
		t.Fatalf("unable to generate block: %v", err)
	}
	...
	// Fully confirm tx3.
	_, err = vw.GenerateBlocks(ctxb, 2)
	if err != nil {
		t.Fatalf("unable to generate block: %v", err)
	}
...
// working
func testLazyNtfnConsumer(miner *rpctest.Harness, vw *rpctest.VotingWallet,
	notifier chainntnfs.TestChainNotifier, scriptDispatch bool, t *testing.T) {
	...
	if _, err := vw.GenerateBlocks(ctxb, 2); err != nil {
		t.Fatalf("unable to generate blocks: %v", err)
	}
...
func testSpendBeforeNtfnRegistration(miner *rpctest.Harness, vw *rpctest.VotingWallet,
	notifier chainntnfs.TestChainNotifier, scriptDispatch bool, t *testing.T) {
	...
	_, heightHint, err := miner.Node.GetBestBlock(ctxb)
	if err != nil {
		t.Fatalf("unable to get current height: %v", err)
	}
	...
	_, spendHeight, err := miner.Node.GetBestBlock(ctxb)
	if err != nil {
		t.Fatalf("unable to get current height: %v", err)
	}

	// When testing via scriptDispatch we ignore the outpoint.
	if scriptDispatch {
		...
		for _, client := range spendClients {
			...
			case ntfn := <-client.Spend:
				...
				checkNotificationFields(
					ntfn, outpoint, spenderHash, spendHeight, t,
				)
			case <-time.After(30 * time.Second):
				t.Fatalf("spend ntfn never received")
			}
...
func testCancelSpendNtfn(node *rpctest.Harness, vw *rpctest.VotingWallet,
	notifier chainntnfs.TestChainNotifier, scriptDispatch bool, t *testing.T) {
	...
	// ourselves.
	outpoint, output, privKey := chainntnfs.CreateSpendableOutput(t, node, vw)

	_, currentHeight, err := node.Node.GetBestBlock(ctxb)
	if err != nil {
		t.Fatalf("unable to get current height: %v", err)
	}
	...
	// Broadcast our spending transaction.
	spenderHash, err := node.Node.SendRawTransaction(ctxb, spendingTx, true)
	if err != nil {
		t.Fatalf("unable to broadcast tx: %v", err)
	}
...
func testCancelEpochNtfn(node *rpctest.Harness, vw *rpctest.VotingWallet,
	notifier chainntnfs.TestChainNotifier, t *testing.T) {
...
func testReorgConf(miner *rpctest.Harness, vw *rpctest.VotingWallet,
	notifier chainntnfs.TestChainNotifier, scriptDispatch bool, t *testing.T) {
	...
	_, nodeHeight1, err := miner.Node.GetBestBlock(ctxb)
	if err != nil {
		t.Fatalf("unable to get current blockheight %v", err)
	}

	_, nodeHeight2, err := miner2.Node.GetBestBlock(ctxb)
	if err != nil {
		t.Fatalf("unable to get current blockheight %v", err)
	}
	...
	err = rpctest.RemoveNode(ctxb, miner, miner2)
	if err != nil {
	...
	tx, err := miner.Node.GetRawTransaction(ctxb, txid)
	if err != nil {
		t.Fatalf("unable to get raw tx: %v", err)
	}

	txid, err = miner2.Node.SendRawTransaction(ctxb, tx.MsgTx(), false)
	if err != nil {
...
// the chain.
func testReorgSpend(miner *rpctest.Harness, vw *rpctest.VotingWallet,
	notifier chainntnfs.TestChainNotifier, scriptDispatch bool, t *testing.T) {
	...
	_, minerHeight2, err := miner2.Node.GetBestBlock(ctxb)
	if err != nil {
	...
	spendTxHash, err := miner.Node.SendRawTransaction(ctxb, spendTx, true)
	if err != nil {
	...
	// it.
	if _, err = miner2.Node.SendRawTransaction(ctxb, spendTx, true); err != nil {
		t.Fatalf("unable to broadcast spend tx: %v", err)
	}
	if err := chainntnfs.WaitForMempoolTx(miner, spendTxHash); err != nil {
		t.Fatalf("tx not relayed to miner: %v", err)
...
// being out of date.
func testCatchUpClientOnMissedBlocks(miner *rpctest.Harness, vw *rpctest.VotingWallet,
	notifier chainntnfs.TestChainNotifier, t *testing.T) {
	...
	outdatedHash, outdatedHeight, err := miner.Node.GetBestBlock(ctxb)
	if err != nil {
		t.Fatalf("unable to retrieve current height: %v", err)
	}
	...
	generateBlocks := func() error {
		_, err = vw.GenerateBlocks(ctxb, numBlocks)
		return err
	}
...
// being out of date.
func testCatchUpOnMissedBlocks(miner *rpctest.Harness, vw *rpctest.VotingWallet,
	notifier chainntnfs.TestChainNotifier, t *testing.T) {
	...
	_, bestHeight, err := miner.Node.GetBestBlock(ctxb)
	if err != nil {
		t.Fatalf("unable to get current blockheight %v", err)
	}
...
// out of the chain.
func testCatchUpOnMissedBlocksWithReorg(miner1 *rpctest.Harness, vw *rpctest.VotingWallet,
	notifier chainntnfs.TestChainNotifier, t *testing.T) {
	...
	// The two should be on the same blockheight.
	_, nodeHeight1, err := miner1.Node.GetBestBlock(ctxb)
	if err != nil {
		t.Fatalf("unable to get current blockheight %v", err)
	}
	...
	blocks, err := vw.GenerateBlocks(ctxb, numBlocks)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
	...
	for {
		nodeHash1, _, err := miner1.Node.GetBestBlock(ctxb)
		if err != nil {
			t.Fatalf("unable to get current block hash: %v", err)
		}

		nodeHash2, _, err := miner2.Node.GetBestBlock(ctxb)
		if err != nil {
			t.Fatalf("unable to get current block hash: %v", err)
		}
...
type txNtfnTestCase struct {
	name string
	test func(node *rpctest.Harness, vw *rpctest.VotingWallet, notifier chainntnfs.TestChainNotifier,
...
var txNtfnTests = []txNtfnTestCase{
	// These tests needs come before the others to prevent the reorg
	// messing with stake voting.
	{
		name: "reorg conf",
		test: testReorgConf,
	},
...
var blockCatchupTests = []blockCatchupTestCase{
	// This test needs to come first to prevent it messing with stake
	// transactions for the others.
	{
...
func TestInterfaces(t *testing.T) {
	...
	// that.
	newMiner := func(t *testing.T, notifierType string) (*rpctest.Harness, *rpctest.VotingWallet, func()) {
		...
		vw, err := rpctest.NewVotingWallet(ctxb, miner)
		if err != nil {

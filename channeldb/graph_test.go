...
func createLightningNode(db kvdb.Backend, priv *secp256k1.PrivateKey) (*LightningNode, error) {
...
func TestEdgeInfoUpdates(t *testing.T) {
	...
	// Make sure inserting the policy at this point, before
	// the edge info is added, will fail.
	...
	// Next, attempt to query the channel edges
	// according to the outpoint of the channel.
...
func newEdgePolicy(chanID uint64, db kvdb.Backend,
	...
	return &ChannelEdgePolicy{
		...
		MaxHTLC:                   lnwire.MilliAtom(prand.Int63()),
...
func fillTestGraph(t require.TestingT, graph *ChannelGraph, numNodes,
	...
	for n := 0; n < numNodes-1; n++ {
		...
		for i := 0; i < numChannels; i++ {
			...
			copy(edgeInfo.DecredKey1Bytes[:], node1.PubKeyBytes[:])
			copy(edgeInfo.DecredKey2Bytes[:], node2.PubKeyBytes[:])

	...
	if v, ok := val.(*AMPInvoiceState); ok {
		...
		for i := uint64(0); i < numRecords; i++ {
			...
			(*v)[setID] = InvoiceStateAMP{
				...
				AmtPaid:     lnwire.MilliAtom(amtPaid),
			}
		}
...
func deserializeHtlcs(r io.Reader) (map[CircuitKey]*InvoiceHTLC, error) {
	...
	for {
		...
		htlc.MppTotalAmt = lnwire.MilliAtom(mppTotalAmt)
		if amp != nil && hash != nil {
		...
		for _, ref := range invoicesToDelete {
			...
			if ref.PayAddr != nil {
				...
				if bytes.Equal(key, invoiceKey) {
					...
					// for this invoice.
					// ref: https://github.com/decred/dcrlnd/pull/4285/commits/cbf71b5452fa1d3036a43309e490787c5f7f08dc#r426368127

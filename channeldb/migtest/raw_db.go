...
// 	},
// }
func DumpDB(tx *bolt.Tx, rootKey []byte) error {
...
func dumpBucket(bucket *bolt.Bucket) error {
...
func RestoreDB(tx *bolt.Tx, rootKey []byte, data map[string]interface{}) error {
...
func restoreDB(bucket *bolt.Bucket, data map[string]interface{}) error {
	for k, v := range data {
		key := []byte(k)
...
func VerifyDB(tx *bolt.Tx, rootKey []byte, data map[string]interface{}) error {
	bucket := tx.Bucket(rootKey)
	if bucket == nil {
		return fmt.Errorf("bucket %v not found", string(rootKey))
	}
...
func verifyDB(bucket *bolt.Bucket, data map[string]interface{}) error {
	for k, v := range data {
		key := []byte(k)

...
// traversal.
func (c *ChannelGraph) ForEachNodeChannel(tx *bolt.Tx, nodePub []byte,
	cb func(*bolt.Tx, *ChannelEdgeInfo, *ChannelEdgePolicy,
...
func (c *ChannelGraph) addChannelEdge(tx *bolt.Tx, edge *ChannelEdgeInfo) error {
...
func getChanID(tx *bolt.Tx, chanPoint *wire.OutPoint) (uint64, error) {
	var b bytes.Buffer
...
func delChannelEdge(edges, edgeIndex, chanIndex, zombieIndex,
	nodes *bolt.Bucket, chanID []byte, isZombie bool) error {
...
// true if the updated policy belongs to node1, and false if the policy
// belonged to node2.
func updateEdgePolicy(tx *bolt.Tx, edge *ChannelEdgePolicy) (bool, error) {
	edges := tx.Bucket(edgeBucket)
	if edges == nil {
...
// be nil and a fresh transaction will be created to execute the graph
// traversal.
func (c *ChannelGraph) FetchLightningNode(tx *bolt.Tx, nodePub route.Vertex) (
	*LightningNode, error) {

	var node *LightningNode

	fetchNode := func(tx *bolt.Tx) error {
		// First grab the nodes bucket which stores the mapping from
		// pubKey to node information.
		nodes := tx.Bucket(nodeBucket)
		if nodes == nil {
...
// nodeTraversal is used to traverse all channels of a node given by its public
// key and passes channel information into the specified callback.
func nodeTraversal(tx *bolt.Tx, nodePub []byte, db *DB,
...
// DecredKey1 is the Decred multi-sig key belonging to the first
...
// DecredKey2 is the Decred multi-sig key belonging to the second
// node, that was involved in the funding transaction that originally
// created the channel that this struct represents.
//
// NOTE: By having this method to access an attribute, we ensure we only need
...
// DecredSig1 is the signature using the public key of the first node that was
...
// DecredSig2 is the signature using the public key of the second node that
...
func (c *ChannelAuthProof) IsEmpty() bool {
	return len(c.NodeSig1Bytes) == 0 ||
		...
		len(c.DecredSig1Bytes) == 0 ||
		len(c.DecredSig2Bytes) == 0
}
	...
	if err := c.db.View(func(tx *bolt.Tx) error {
		...
		return chanIndex.ForEach(func(chanPointBytes, chanID []byte) error {
			...
			pkScript, err := genMultiSigP2SH(
				edgeInfo.DecredKey1Bytes[:],
				edgeInfo.DecredKey2Bytes[:],
			)
			if err != nil {
				return err
			}
...
// keys should represent the node public keys of the two parties involved in
// the edge.
func markEdgeZombie(zombieIndex *bolt.Bucket, chanID uint64, pubKey1,
...
func isZombieEdge(zombieIndex *bolt.Bucket,
...
func putChanEdgeInfo(edgeIndex *bolt.Bucket, edgeInfo *ChannelEdgeInfo, chanID [8]byte) error {
	...
	if _, err := b.Write(edgeInfo.DecredKey1Bytes[:]); err != nil {
		return err
	}
	if _, err := b.Write(edgeInfo.DecredKey2Bytes[:]); err != nil {
		return err
	}
...
func deserializeChanEdgeInfo(r io.Reader) (ChannelEdgeInfo, error) {
	...
	if _, err := io.ReadFull(r, edgeInfo.DecredKey1Bytes[:]); err != nil {
		return ChannelEdgeInfo{}, err
	}
	if _, err := io.ReadFull(r, edgeInfo.DecredKey2Bytes[:]); err != nil {
		return ChannelEdgeInfo{}, err
	}
	...
	proof.DecredSig1Bytes, err = wire.ReadVarBytes(r, 0, 80, "sigs")
	if err != nil {
		return ChannelEdgeInfo{}, err
	}
	proof.DecredSig2Bytes, err = wire.ReadVarBytes(r, 0, 80, "sigs")
	if err != nil {
		return ChannelEdgeInfo{}, err
	}
...
func updateEdgePolicyDisabledIndex(edges *bolt.Bucket, chanID uint64,
...
func serializeChanEdgePolicy(w io.Writer, edge *ChannelEdgePolicy,
	...
	if err := binary.Write(w, byteOrder, uint64(edge.FeeBaseMAtoms)); err != nil {
		return err
	}
...
func deserializeChanEdgePolicy(r io.Reader,
	...
	edge.FeeBaseMAtoms = lnwire.MilliAtom(n)

...
func randInvoice(value lnwire.MilliAtom) (*Invoice, error) {
	var (
...
func TestInvoiceCancelSingleHtlc(t *testing.T) {
	...
	testInvoice := &Invoice{
		Htlcs: map[CircuitKey]*InvoiceHTLC{},
		Terms: ContractTerm{
			Value:           lnwire.NewMAtomsFromAtoms(10000),
			Features:        emptyFeatures,
...
func TestScanInvoices(t *testing.T) {
	...
	for i := 1; i <= numInvoices; i++ {
		invoice, err := randInvoice(lnwire.MilliAtom(i))
		require.NoError(t, err)
...
// TestQueryInvoices ensures that we can properly query the invoice database
// for invoices using different types of queries.
...
func getUpdateInvoice(amt lnwire.MilliAtom) InvoiceUpdateCallback {
...
func makeAMPInvoiceHTLC(amt lnwire.MilliAtom, setID [32]byte,
...
// settles the invoice with the given amount.
func updateAcceptAMPHtlc(id uint64, amt lnwire.MilliAtom,
...
func TestUnexpectedInvoicePreimage(t *testing.T) {
	...
	invoice, err := randInvoice(lnwire.MilliAtom(100))
	require.NoError(t, err)

...
func channelCommitmentOption(height uint64, localBalance,
	remoteBalance lnwire.MilliAtom, local bool) testChannelOption {
...
func TestOpenChannelPutGetDelete(t *testing.T) {
	...
	nextRevKeyPub := nextRevKey.PubKey()
...
func TestBalanceAtHeight(t *testing.T) {
	...
	tests := []struct {
		name                  string
		targetHeight          uint64
		expectedLocalBalance  lnwire.MilliAtom
		expectedRemoteBalance lnwire.MilliAtom
		expectedError         error
	}{
		{

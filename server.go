...
func newServer(cfg *Config, listenAddrs []net.Addr,
	...
	s.fundingMgr, err = funding.NewFundingManager(funding.Config{
		...
		RequiredRemoteDelay: func(chanAmt dcrutil.Amount) uint16 {
			...
			// for small channels, to maxRemoteDelay blocks for
			// channels of size MaxFundingAmount.
...
func (s *server) initialPeerBootstrap(ctx context.Context, ignore map[autopilot.NodeID]struct{},
...
func (s *server) BroadcastMessage(skips map[route.Vertex]struct{},
	...
	for _, sPeer := range s.peersByPub {
		if skips != nil {
			if _, ok := skips[sPeer.PubKey()]; ok {
				srvrLog.Debugf("Skipping %x in broadcast",
					sPeer.PubKey())
				continue
			}
		}
	...
	srvrLog.Debugf("Broadcasting %v messages to %d peers with %d in skipList",
		len(msgs), len(peers), len(skips))

	// Iterate over all known peers, dispatching a go routine to enqueue
	// all messages to each of peers.
	var wg sync.WaitGroup
	for _, sPeer := range peers {
		srvrLog.Debugf("Sending messages [%s] to peer %s",
			lnwire.MessagesTypesLogger(msgs), sPeer)
...
//
// NOTE: This function is safe for concurrent access.
func (s *server) FindPeer(peerKey *secp256k1.PublicKey) (*peer.Brontide, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *server) fetchNodeAdvertisedAddrs(pub *secp256k1.PublicKey) ([]net.Addr, error) {

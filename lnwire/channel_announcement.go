...
type ChannelAnnouncement struct {
	...
	// This signatures are used by nodes in order to create cross
	// references between node's channel and node. Requiring the decred
...
func (a *ChannelAnnouncement) Encode(w *bytes.Buffer, pver uint32) error {
	...
	if err := WriteSig(w, a.DecredSig1); err != nil {
		return err
	}

	if err := WriteSig(w, a.DecredSig2); err != nil {
		return err
	}
	...
	if err := WriteBytes(w, a.DecredKey1[:]); err != nil {
		return err
	}

	if err := WriteBytes(w, a.DecredKey2[:]); err != nil {
		return err
	}
...
func (a *ChannelAnnouncement) DataToSign() ([]byte, error) {
	...
	if err := WriteBytes(buf, a.DecredKey1[:]); err != nil {
		return nil, err
	}

	if err := WriteBytes(buf, a.DecredKey2[:]); err != nil {
		return nil, err
	}

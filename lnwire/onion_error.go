...
func NewFailIncorrectDetails(amt MilliAtom,
...
// Amount is the value of the extended HTLC.
func (f *FailIncorrectDetails) Amount() MilliAtom {
	return f.amount
}
...
func (f *FailIncorrectDetails) Encode(w *bytes.Buffer, pver uint32) error {
	if err := WriteMilliAtom(w, f.amount); err != nil {
		return err
	}
...
func (f *FailAmountBelowMinimum) Encode(w *bytes.Buffer, pver uint32) error {
	if err := WriteMilliAtom(w, f.HtlcMAtoms); err != nil {
		return err
	}
...
func (f *FailFinalIncorrectHtlcAmount) Encode(w *bytes.Buffer,
	...
	return WriteMilliAtom(w, f.IncomingHTLCAmount)
}

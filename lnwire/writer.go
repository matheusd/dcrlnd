...
func WriteSatoshi(buf *bytes.Buffer, amount dcrutil.Amount) error {
	return WriteUint64(buf, uint64(amount))
}

// WriteMilliAtom appends the MilliAtom value to the provided buffer.
func WriteMilliAtom(buf *bytes.Buffer, amount MilliAtom) error {
	return WriteUint64(buf, uint64(amount))
}

// WritePublicKey appends the compressed public key to the provided buffer.
func WritePublicKey(buf *bytes.Buffer, pub *secp256k1.PublicKey) error {
	if pub == nil {

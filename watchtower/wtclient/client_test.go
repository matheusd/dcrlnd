...
func (m *mockNet) AuthDial(localPriv *secp256k1.PrivateKey, netAddr *lnwire.NetAddress,
...
type mockChannel struct {
	...
	revSK     *secp256k1.PrivateKey
	revPK     *secp256k1.PublicKey
	revKeyLoc keychain.KeyLocator

	toRemoteSK     *secp256k1.PrivateKey
	toRemotePK     *secp256k1.PublicKey
	toRemoteKeyLoc keychain.KeyLocator

	toLocalPK *secp256k1.PublicKey // only need to generate to-local script

	dustLimit lnwire.MilliAtom
	csvDelay  uint32
}

func newMockChannel(t *testing.T, signer *wtmock.MockSigner,
	localAmt, remoteAmt lnwire.MilliAtom) *mockChannel {
...
func (c *mockChannel) createRemoteCommitTx(t *testing.T) {
	...
	commitTxn := &wire.MsgTx{
		Version: uint16(c.commitHeight + 2),
	}
	...
	if c.remoteBalance >= c.dustLimit {
		commitTxn.TxOut = append(commitTxn.TxOut, &wire.TxOut{
			Value:    int64(c.remoteBalance.ToAtoms()),
	...
	if c.localBalance >= c.dustLimit {
		commitTxn.TxOut = append(commitTxn.TxOut, &wire.TxOut{
			Value:    int64(c.localBalance.ToAtoms()),
...
func (c *mockChannel) sendPayment(t *testing.T, amt lnwire.MilliAtom) {
	t.Helper()
...
func (c *mockChannel) receivePayment(t *testing.T, amt lnwire.MilliAtom) {
	t.Helper()
...
func (h *testHarness) makeChannel(id uint64,
	localAmt, remoteAmt lnwire.MilliAtom) {
...
func (h *testHarness) sendPayments(id, from, to uint64,
	amt lnwire.MilliAtom) []blob.BreachHint {
...
func (h *testHarness) removeTower(pubKey *secp256k1.PublicKey, addr net.Addr) {
	h.t.Helper()
...
const (
	localBalance  = lnwire.MilliAtom(100000000)
	remoteBalance = lnwire.MilliAtom(200000000)
)
	...
	},
	{
		...
		fn: func(h *testHarness) {
			var (
				...
				paymentAmt = lnwire.MilliAtom(200000)

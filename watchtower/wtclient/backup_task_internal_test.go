...
func makeAddrSlice(size int) []byte {
	if size > 75 {
		// This is needed because decred requires a valid script in the
		// pkscript. To simpify tests, we generate a dummy OP_RETURN
		// with a data push so that the total pkscript size is the
		// parametrized size.
		panic("wrong usage")
	}
	addr := make([]byte, size)
	...
	addr[1] = byte(size - 2)
	if _, err := io.ReadFull(rand.Reader, addr[2:]); err != nil {
...
func genTaskTest(
	...
	return backupTaskTest{
		...
		expTotalAmt:      dcrutil.Amount(toLocalAmt + toRemoteAmt),
...
// that the backup task can be rescheduled if necessary. Finally, we assert
// that the backup task is able to encrypt a valid justice kit, and that we can

...
func (r *rpcServer) addDeps(s *server, macService *macaroons.Service,
	...
	routerBackend := &routerrpc.RouterBackend{
		...
		DefaultFinalCltvDelta:  uint16(r.cfg.Decred.TimeLockDelta),
...
func addrPairsToOutputs(addrPairs map[string]int64,
	netParams *chaincfg.Params) ([]*wire.TxOut, error) {
...
func (r *rpcServer) sendCoinsOnChain(paymentMap map[string]int64,
	feeRate chainfee.AtomPerKByte, minConfs int32,
	label, fromAccount string) (*chainhash.Hash, error) {
	...
	// Note(decred): This section is commented out because the wallet does not
	// implement CreateSimpleTx. Anchor commitments are disabled in mainnet,
	// therefore this isn't critical.
	...
	tx, err := r.server.cc.Wallet.SendOutputs(
		outputs, feeRate, minConfs, label, fromAccount,
	)
	if err != nil {
		return nil, err
	}
...
func (r *rpcServer) EstimateFee(ctx context.Context,
	...
	err = wallet.WithCoinSelectLock(func() error {
		tx, err = wallet.CreateSimpleTx(outputs, feePerKB, minConfs, true)
		return err
	})
	if err != nil {
		return nil, err
...
func (r *rpcServer) SendCoins(ctx context.Context,
	...
	targetAddr, err := stdaddr.DecodeAddress(
	...
	if in.SendAll {
		...
		sweepTxPkg, err := sweep.CraftSweepAllTx(
			feePerKB, uint32(bestHeight), nil, targetAddr, wallet,
		...
		var reservedVal dcrutil.Amount
		err = wallet.WithCoinSelectLock(func() error {
			var err error
		...
		if err == lnwallet.ErrReservedValueInvalidated {
			...
			changeAddr, err := r.server.cc.Wallet.NewAddress(
				lnwallet.PubKeyHash, true, lnwallet.DefaultAccountName,
			)
			if err != nil {
				return nil, err
			}
			...
			sweepTxPkg, err = sweep.CraftSweepAllTx(
				feePerKB, uint32(bestHeight), outputs,
		...
		err := wallet.WithCoinSelectLock(func() error {
			newTXID, err := r.sendCoinsOnChain(
				paymentMap, feePerKB, minConfs, label, in.Account,
			)
			if err != nil {
				return err
			}
...
func (r *rpcServer) SendMany(ctx context.Context,
	...
	err = wallet.WithCoinSelectLock(func() error {
		...
		sendManyTXID, err := r.sendCoinsOnChain(
			in.AddrToAmount, feePerKB, minConfs, label, in.Account,
		)
		if err != nil {
			return err
		}
...
func (r *rpcServer) WalletBalance(ctx context.Context,
	...
	var totalBalance, confirmedBalance, unconfirmedBalance dcrutil.Amount
	...
	// chain.
	//
	// Note(decred): this is commented out in dcrlnd because
	// ListLeasedOutputs is not implemented in dcrwallet.
	var lockedBalance dcrutil.Amount
	// leases, err := r.server.cc.Wallet.ListLeasedOutputs()
	leases, err := []*lnwallet.LockedOutput{}, error(nil)
	if err != nil {
		return nil, err
	}
...
func (r *rpcServer) CalcPaymentStats(ctx context.Context,
	...
	stats, err := r.server.miscDB.CalcPaymentStats()
	if err != nil {
		return nil, err
	}
...
func (r *rpcServer) UpdateChannelPolicy(ctx context.Context,
	...
	rpcsLog.Debugf("[updatechanpolicy] updating channel policy base_fee=%v, "+
		...
		req.BaseFeeMAtoms, feeRateFixed, req.TimeLockDelta,
		minHtlc, maxHtlc,

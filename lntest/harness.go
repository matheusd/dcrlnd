...
type NetworkHarness struct {
	...
	votingWallet       *walletTest
	votingWalletClient *rpcclient.Client
...
// setupVotingWallet sets up a dcrwallet voting wallet, so that the simnet used
// for tests can advance past SVH.
func (n *NetworkHarness) setupVotingWallet() error {

	if _, err := n.Miner.Node.Generate(32); err != nil {
		return err
	}

	caFile, err := ioutil.TempFile("", "lnd-voting-wallet-ca.cert")
	if err != nil {
		return err
	}

	_, err = caFile.Write([]byte(n.rpcConfig.Certificates))
	if err != nil {
		caFile.Close()
		return err
	}
	...
	dataDir, err := ioutil.TempDir("", "lnd-voting-wallet")
	if err != nil {
		return err
	}

	certFile := path.Join(dataDir, "rpc.cert")
	keyFile := path.Join(dataDir, "rpc.key")

	// Generate wallet cert pair
	...
	extra := []string{"--nogrpc"}
	cfg, err := newWalletConfig(dataDir, caFileName, certFile, keyFile, extra)
	if err != nil {
		return err
	}
	cfg.rpcListen = "127.0.0.1:29557"
	cfg.rpcConnect = n.rpcConfig.Host
	cfg.rpcUser = n.rpcConfig.User
	cfg.rpcPass = n.rpcConfig.Pass

	w, err := newWallet(cfg, dataDir)
	if err != nil {
		return err
	}
	...
	walletRPCConf := w.config.rpcConnConfig()
	for i := 0; i < 400; i++ {
		if walletClient, err = rpcclient.New(&walletRPCConf, nil); err != nil {
	...
	n.votingWalletClient = walletClient

	// The wallet does not currently have an endpoint that indicates rescan
	// completeness, so wait a bit.
	time.Sleep(time.Second * 8)

	_, desiredHeight, err := n.Miner.Node.GetBestBlock()
	if err != nil {
		return err
	}

	err = WaitPredicate(func() bool {
		count, err := walletClient.GetBlockCount()
		if err != nil {
			return false
		}
		return count == desiredHeight
	}, time.Second*30)
	if err != nil {
		return err
	}

	// Get the current balance, send 2 DCR to the wallet (which will be used to
	// buy simnet tickets), then wait until that is recognized by the wallet.
	prevBalance, err := walletClient.GetBalance("default")
	if err != nil {
		return err
	}
	addr, err := walletClient.GetNewAddress("default")
	if err != nil {
		return err
	}
	addrScript, err := txscript.PayToAddrScript(addr)
	...
	coinAmount := float64(2)
	output := &wire.TxOut{
		PkScript: addrScript,
		Value:    int64(coinAmount+1) * 1e8,
	}
	_, err = n.Miner.SendOutputs([]*wire.TxOut{output}, 1e4)
	if err != nil {
		return err
	}

	if _, err := n.Miner.Node.Generate(2); err != nil {
		return err
	}

	err = WaitPredicate(func() bool {
		currBalance, err := walletClient.GetBalance("default")
		if err != nil {
			return false
		}
		return currBalance.Balances[0].Total >= prevBalance.Balances[0].Total+coinAmount
	}, time.Second*10)
	if err != nil {
		return err
	}

	_, err = n.FillStakepool()
	return err
}

// PrintStakepool is a helper function to debug the stakepool situation for the
// voting wallet of the harness. It is meant to ONLY be used when debugging
// tests. It panics in case of problems.
func (n *NetworkHarness) PrintStakepool(msg string) {
	stakeInfo, err := n.votingWalletClient.GetStakeInfo()
	if err != nil {
		panic(err)
	}

	bc, err := n.votingWalletClient.GetBlockCount()
	if err != nil {
		panic(err)
	}

	fmt.Printf("_______ height %d poolsize %d immature %d allmempool %d (%s)\n", bc,
		stakeInfo.PoolSize, stakeInfo.Immature, stakeInfo.AllMempoolTix, msg)
}

// FillStakepool mines as many blocks as needed while purchasing tickets to
// fill the stakepool of the voting wallet with enough tickets so that it can
// continue functioning for a few blocks without requiring new purchases.
func (n *NetworkHarness) FillStakepool() (uint32, error) {
	...
	ticketsPerBlock := 20
	nbTickets := 320 - int(stakeInfo.PoolSize)
	if nbTickets <= 0 {
		// No tickets needed.
		return 0, nil
	}

	addr, err := n.votingWalletClient.GetNewAddress("default")
	if err != nil {
		return 0, err
	}

	minConf := 0
	maxAmount := dcrutil.Amount(2e8)

	nbBlocks := uint32(0)
	// firstBlock := true

	genBlocks := func(nb uint32) error {
		_, err := n.Generate(nb)
		if err != nil {
			return err
		}
	...
	for nbTickets > 5 {
		ticketsToPurchase := ticketsPerBlock
		if nbTickets < ticketsPerBlock {
			ticketsToPurchase = nbTickets
		}

		_, err = n.votingWalletClient.PurchaseTicket("default", maxAmount,
			&minConf, addr, &ticketsToPurchase, nil, nil, nil, nil, nil)
		if err != nil {
			if strings.Index(err.Error(), "not enough funds to meet stake difficulty") > -1 {
				// Just mine an extra block so the wallet will use the next stakediff
				if err := genBlocks(1); err != nil {
					return nbBlocks, err
				}
				continue
			}
		...
		nbTickets -= ticketsToPurchase
		nbTickets += 5 // +5 to account for the last block
	}

	// Mine the rest of tickets (if there are any outstanding).
	if err := genBlocks(2); err != nil {
		return nbBlocks, err
	}
...
func (n *NetworkHarness) Generate(nb uint32) ([]*chainhash.Hash, error) {
	...
	_, startHeight, err := n.Miner.Node.GetBestBlock()
	if err != nil {
		return nil, err
	}
	...
	for i := uint32(0); i < nb; i++ {
		genHashes, err := n.Miner.Node.Generate(1)
		if err != nil {
			return nil, err
		}
		hashes[i] = genHashes[0]

		time.Sleep(time.Millisecond * 200)
	}

	_, endHeight, err := n.Miner.Node.GetBestBlock()
	if err != nil {
		return nil, err
	}

	if endHeight-startHeight != int64(nb) {
		return nil, fmt.Errorf("less than requested (%d) number of blocks "+
			"generated (%d) starting at height %d", nb, endHeight-startHeight,
			startHeight)
	}
...
// GenerateWithTickets generates a large number of blocks (at least 20) while
// also periodically purchasing tickets, such that the chain can continue to
// advance after it passes SVH.
func (n *NetworkHarness) GenerateWithTickets(nb uint32) error {
	if nb < 20 {
		return fmt.Errorf("less than 20 blocks, use generate()")
	}
	...
	if endHeight-startHeight != int64(nb) {
		return fmt.Errorf("less than requested (%d) number of blocks "+
			"generated (%d) with tickets starting at height %d", nb, endHeight-startHeight,
			startHeight)
	}

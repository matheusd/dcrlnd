...
func testSingleHopSendToRouteCase(net *lntest.NetworkHarness, t *harnessTest,
	...
	// reaches Dave.
	_, minerHeight, err := net.Miner.Node.GetBestBlock(testctx.New(t.t))
	if err != nil {
	...
	setMPPFields := func(i int) {
		...
		hop.MppRecord = &lnrpc.MPPRecord{
			...
			TotalAmtMAtoms: paymentAmtSat * 1000,
		}
	}
	...
	for i, p := range paymentsResp.Payments {
		...
		if p.ValueAtoms != paymentAmtSat {
			t.Fatalf("incorrect payment amt for payment %d, "+
				"want: %d, got: %d",
				i, paymentAmtSat, p.ValueAtoms)
		}
		...
		if hop.MppRecord.TotalAmtMAtoms != paymentAmtSat*1000 {
	...
	for i, inv := range invoicesResp.Invoices {
		...
		if inv.AmtPaidAtoms != paymentAmtSat {
			t.Fatalf("incorrect payment amt for invoice %d, "+
				"want: %d, got %d",
				i, paymentAmtSat, inv.AmtPaidAtoms)
		}
	}
...
func testMultiHopSendToRoute(net *lntest.NetworkHarness, t *harnessTest) {
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	net.SendCoins(ctxt, t.t, dcrutil.AtomsPerCoin, net.Bob)
	...
	for i, rHash := range rHashes {
		...
		route.Hops[len(route.Hops)-1].MppRecord = &lnrpc.MPPRecord{
			...
			TotalAmtMAtoms: int64(
				lnwire.NewMAtomsFromAtoms(paymentAmt),
			),
		}
...
func testSendToRouteErrorPropagation(net *lntest.NetworkHarness, t *harnessTest) {
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	net.SendCoins(ctxt, t.t, dcrutil.AtomsPerCoin, charlie)
...
func testUpdateChannelPolicyForPrivateChannel(net *lntest.NetworkHarness,
	...
	err = carol.WaitForNetworkChannelOpen(ctxb, chanPointAliceBob)
...
func testQueryRoutes(net *lntest.NetworkHarness, t *harnessTest) {
	...
	for i, route := range routesRes.Routes {
		...
		if route.TotalFees != route.TotalFeesMAtoms/mSat { // nolint:staticcheck
			t.Fatalf("route %v: total fees %v (msat) does not "+
				"round down to %v (sat)",
				i, route.TotalFeesMAtoms, route.TotalFees) // nolint:staticcheck
		}
		if route.TotalFeesMAtoms != int64(expectedTotalFeesMSat) {
			t.Fatalf("route %v: total fees in msat expected %v got %v",
				i, expectedTotalFeesMSat, route.TotalFeesMAtoms)
		}

		if route.TotalAmt != route.TotalAmtMAtoms/mSat { // nolint:staticcheck
			t.Fatalf("route %v: total amt %v (msat) does not "+
				"round down to %v (sat)",
				i, route.TotalAmtMAtoms, route.TotalAmt) // nolint:staticcheck
		}
		if route.TotalAmtMAtoms != int64(expectedTotalAmtMSat) {
			t.Fatalf("route %v: total amt in msat expected %v got %v",
				i, expectedTotalAmtMSat, route.TotalAmtMAtoms)
		}
		...
		for j, hop := range route.Hops[:len(route.Hops)-1] {
			...
			if hop.Fee != hop.FeeMAtoms/mSat { // nolint:staticcheck
				t.Fatalf("route %v hop %v: fee %v (msat) does not "+
					"round down to %v (sat)",
					i, j, hop.FeeMAtoms, hop.Fee) // nolint:staticcheck
			}
			if hop.FeeMAtoms != int64(feePerHopMSat) {
				t.Fatalf("route %v hop %v: fee in msat expected %v got %v",
					i, j, feePerHopMSat, hop.FeeMAtoms)
			}

			if hop.AmtToForward != hop.AmtToForwardMAtoms/mSat { // nolint:staticcheck
				t.Fatalf("route %v hop %v: amt to forward %v (msat) does not "+
					"round down to %v (sat)",
					i, j, hop.AmtToForwardMAtoms, hop.AmtToForward) // nolint:staticcheck
			}
			if hop.AmtToForwardMAtoms != int64(expectedAmtToForwardMSat) {
				t.Fatalf("route %v hop %v: amt to forward in msat "+
					"expected %v got %v",
					i, j, expectedAmtToForwardMSat, hop.AmtToForwardMAtoms)
			}
		}
		...
		if hop.Fee != 0 || hop.FeeMAtoms != 0 { // nolint:staticcheck
			t.Fatalf("route %v hop %v: fee expected 0 got %v (sat) %v (msat)",
				i, len(route.Hops)-1, hop.Fee, hop.FeeMAtoms) // nolint:staticcheck
		}

		if hop.AmtToForward != hop.AmtToForwardMAtoms/mSat { // nolint:staticcheck
			t.Fatalf("route %v hop %v: amt to forward %v (msat) does not "+
				"round down to %v (sat)",
				i, len(route.Hops)-1, hop.AmtToForwardMAtoms, hop.AmtToForward) // nolint:staticcheck
		}
		if hop.AmtToForwardMAtoms != paymentAmt*mSat {
			t.Fatalf("route %v hop %v: amt to forward in msat "+
				"expected %v got %v",
				i, len(route.Hops)-1, paymentAmt*mSat, hop.AmtToForwardMAtoms)
		}
	}
...
func testMissionControlImport(t *testing.T, node *lntest.HarnessNode,
	...
	require.Zero(t, resp1.History.FailAmtMAtoms)
	...
	require.Equal(t, importHistory.FailAmtMAtoms, resp2.History.FailAmtMAtoms)

	// Finally, check that we will fail if inconsistent sat/msat values are
	// set.
	importHistory.FailAmtAtoms = amount * 2
	_, err = node.RouterClient.XImportMissionControl(ctxb, req)
...
func testRouteFeeCutoff(net *lntest.NetworkHarness, t *harnessTest) {
	...
	feeLimitFixed := &lnrpc.FeeLimit{
		Limit: &lnrpc.FeeLimit_Fixed{
			Fixed: int64(carolFee.ToAtoms()) - 1,
		},
	}

...
func testSingleHopPTLC(net *lntest.NetworkHarness, t *harnessTest) {
	...
	ptlcHash := sha256.Sum256(secp256k1.PrivKeyFromBytes(preimage).PubKey().SerializeCompressed())
	t.Logf("preimage: %x", preimage)
	t.Logf("std hash: %x", sha256.Sum256(preimage))
	t.Logf("pnt hash: %x", ptlcHash)
	t.Logf("res hash: %x", invoiceResp.RHash)
	...
	t.Logf("IIIIII Gonna send first")
	...
	t.Logf("IIIIIII gonna send second")

...
func testForwardInterceptor(net *lntest.NetworkHarness, t *harnessTest) {
	...
	go func() {
		...
		for _, tc := range testCases {
			attempt, err := testContext.sendAliceToCarolPayment(
				context.Background(), tc.invoice.ValueMAtoms, tc.invoice.RHash)
	...
	for i := 0; i < len(testCases); i++ {
		...
		case request := <-interceptedChan:
			...
			require.NotZero(t.t, request.IncomingAmountMAtoms)
			...
			require.Less(
				t.t,
				request.OutgoingAmountMAtoms,
				request.IncomingAmountMAtoms,
			)
	...
	for _, testCase := range testCases {
		if testCase.shouldHold {
			...
			expectedAmt := testCase.invoice.ValueMAtoms
			...
			if foundPayment.ValueMAtoms != expectedAmt ||
				foundPayment.Status != lnrpc.Payment_IN_FLIGHT {

				t.Fatalf("expected to find in flight payment for"+
					"amount %v, %v", testCase.invoice.ValueMAtoms, foundPayment.Status)
			}
		}
	}
...
func (c *interceptorTestContext) openChannel(from, to *lntest.HarnessNode, chanSize dcrutil.Amount) {
	ctxb := context.Background()
...
func (c *interceptorTestContext) sendAliceToCarolPayment(ctx context.Context,
	amtMAtoms int64, paymentHash []byte) (*lnrpc.HTLCAttempt, error) {

	// Build a route from alice to carol.
	route, err := c.buildRoute(ctx, amtMAtoms, []*lntest.HarnessNode{c.bob, c.carol})
	if err != nil {
		return nil, err
	}
...
func (c *interceptorTestContext) buildRoute(ctx context.Context, amtMAtoms int64, hops []*lntest.HarnessNode) (
	*lnrpc.Route, error) {

	...
	go func() {
		...
		for _, tc := range testCases {
			attempt, err := testContext.sendAliceToCarolPayment(
				context.Background(), tc.invoice.ValueMAtoms,
		...
		if testCase.shouldHold {
			...
			require.Equal(t.t, lnrpc.Payment_IN_FLIGHT,
				foundPayment.Status, "expected payment to be "+
					"in flight")
			require.Equal(t.t, expectedAmt, foundPayment.ValueMAtoms,
...
func (c *interceptorTestContext) openChannel(from, to *lntest.HarnessNode,
	chanSize dcrutil.Amount) {

	c.net.SendCoins(c.t.t, dcrutil.AtomsPerCoin, from)
...
func (c *interceptorTestContext) sendAliceToCarolPayment(ctx context.Context,
	...
	route, err := c.buildRoute(
		ctx, amtMAtoms, []*lntest.HarnessNode{c.bob, c.carol},
		paymentAddr,
	)
	if err != nil {
		return nil, err
...
func (c *interceptorTestContext) buildRoute(ctx context.Context, amtMAtoms int64,

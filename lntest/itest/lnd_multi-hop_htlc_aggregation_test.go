...
func testMultiHopHtlcAggregation(net *lntest.NetworkHarness, t *harnessTest,
	...
	if c == lnrpc.CommitmentType_ANCHORS || c == lnrpc.CommitmentType_SCRIPT_ENFORCED_LEASE {
	...
	if c != lnrpc.CommitmentType_SCRIPT_ENFORCED_LEASE {
		...
		// output.
		_, err = net.Generate(defaultCSV - 2)
		require.NoError(t.t, err)
		...
		require.NoError(t.t, err)
		bobCommitSweep, err := net.Miner.Node.GetRawTransaction(
			testctx.New(t), bobCommitSweepHash,
		)
		require.NoError(t.t, err)
	...
	secondLevelSweep, err := net.Miner.Node.GetRawTransaction(testctx.New(t), bobSweep)
	require.NoError(t.t, err)

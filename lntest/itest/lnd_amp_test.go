...
func testSendPaymentAMPInvoiceCase(net *lntest.NetworkHarness, t *harnessTest,
	...
	require.Equal(t.t, int64(0), rpcInvoice.AmtPaidAtoms)
	require.Equal(t.t, int64(0), rpcInvoice.AmtPaidMAtoms)
	...
	require.Equal(t.t, int64(paymentAmt), rpcInvoice.AmtPaidAtoms)
	require.Equal(t.t, int64(paymentAmt*1000), rpcInvoice.AmtPaidMAtoms)
...
func testSendPaymentAMPInvoiceRepeat(net *lntest.NetworkHarness,
	...
	chanAmt := dcrutil.Amount(100_000)
	...
	require.Equal(t.t, paymentAmt, int(invoiceNtfn.AmtPaidAtoms))
	...
	// paid.
	require.True(t.t, invoiceNtfn.Settled) // nolint:staticcheck
	require.Equal(t.t, paymentAmt*2, int(invoiceNtfn.AmtPaidAtoms))
	...
	require.Equal(t.t, paymentAmt*2, int(backlogInv.AmtPaidAtoms))
}
...
func testSendToRouteAMP(net *lntest.NetworkHarness, t *harnessTest) {
	...
	const (
		paymentAmt = dcrutil.Amount(300000)
		numShards  = 3
	...
	ctx.alice.LogPrintf("======================== gonna start %s", shardAmt)
	...
	sendShard := func(i int, hops []*lntest.HarnessNode) {
		...
		hop.MppRecord = &lnrpc.MPPRecord{
			PaymentAddr:    payAddr,
			TotalAmtMAtoms: int64(paymentAmt * 1000),
		}
...
func assertInvoiceEqual(t *testing.T, a, b *lnrpc.Invoice) {
	...
	require.Equal(t, a.ValueMAtoms, b.ValueMAtoms)
	...
	require.Equal(t, a.AmtPaidAtoms, b.AmtPaidAtoms)
	require.Equal(t, a.AmtPaidMAtoms, b.AmtPaidMAtoms)
	...
	for i := range a.Htlcs {
		...
		require.Equal(t, htlcA.AmtMAtoms, htlcB.AmtMAtoms)
		...
		require.Equal(t, htlcA.MppTotalAmtMAtoms, htlcB.MppTotalAmtMAtoms)

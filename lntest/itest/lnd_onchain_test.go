...
func testAnchorReservedValue(net *lntest.NetworkHarness, t *harnessTest) {
	...
	minerAddr, err := net.Miner.NewAddress(testctx.New(t))
	require.NoError(t.t, err)
	...
	assertNumPendingChannels(t, alice, 0, 0, 0, 0)
...
func genAnchorSweep(t *harnessTest, net *lntest.NetworkHarness,
	aliceAnchor *sweptOutput, anchorCsv uint32) *dcrutil.Tx {
	...
	aliceAnchorTxIn := func() wire.TxIn {
		for _, txIn := range aliceAnchor.SweepTx.TxIn {
	...
	aliceAnchorTxIn.SignatureScript = nil
	...
	scriptVersion, addrScript := minerAddr.PaymentScript()
...
func testAnchorThirdPartySpend(net *lntest.NetworkHarness, t *harnessTest) {
	// This test requires an rpctest func (GenerateAndSubmitBlock) to
	// perform something that is not currently available in decred: namely,
	// to mine a block with _only_ a specific set of transactions. This is
	// hard to do in decred, due to the requirements of the staking
	// subsystem.
	//
	// This function is used so that the test can enforce that the
	// transaction that redeems the anchor output from the force-closed
	// channel is NOT mined (along with the force-close) and instead is
	// double spent by someone else.
	//
	// Given that anchor outputs are currently disabled in mainnet, this
	// test is skipped.
	t.Skipf("Test disabled in dcrlnd")
	...
	sweepTxns, err := getNTxsFromMempool(
		net.Miner.Node, expectedTxns, minerMempoolTimeout,
	)
	...
	commitTxn, err := net.Miner.Node.GetRawTransaction(
		testctx.New(t), forceCloseTxID,
	)
	if err != nil {
	...
	_, err = net.Miner.GenerateAndSubmitBlock(
		[]*dcrutil.Tx{commitTxn}, -1, emptyTime,
	)
	if err != nil {
	...
	_, err = net.Miner.GenerateAndSubmitBlock(
		[]*dcrutil.Tx{thirdPartyAnchorSweep}, -1, emptyTime,
	)
	if err != nil {
		t.Fatalf("unable to generate block: %v", err)
	}
	...
	assertSpendingTxInMempool(
		t, net.Miner.Node, minerMempoolTimeout, commitSweepOp,
	)

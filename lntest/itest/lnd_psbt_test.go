...
func testPsbtChanFundingExternal(net *lntest.NetworkHarness, t *harnessTest) {
	...
	// No transaction should have been published yet.
	mempool, err := net.Miner.Node.GetRawMempool(testctx.New(t.t), jsonrpctypes.GRMRegular)
	require.NoError(t.t, err)
	require.Equal(t.t, 0, len(mempool))
	...
	txHash := finalTx.TxHash()
	_, err = net.Miner.Node.SendRawTransaction(testctx.New(t.t), &finalTx, false)
	require.NoError(t.t, err)
...
func runSignPsbt(t *harnessTest, net *lntest.NetworkHarness,
	...
	xpub, err := hdkeychain.NewKeyFromString(account.Xpub, harnessNetParams)
	require.NoError(t.t, err)
	...
	change, err := xpub.Child(changeIndex) // nolint:staticcheck
	require.NoError(t.t, err)

	// At an index that we are certainly not watching in the wallet.
	addrKey, err := change.Child(addrIndex) // nolint:staticcheck
	require.NoError(t.t, err)

	addrPubKey, err := stdaddr.NewAddressPubKeyEcdsaSecp256k1V0Raw(addrKey.SerializedPubKey(), harnessNetParams)
	require.NoError(t.t, err)
	pubKeyHashAddr := addrPubKey.AddressPubKeyHash()

	version, pkScript := pubKeyHashAddr.PaymentScript()
	...
	prevTx.Version = input.LNTxVersion
	...
	packet.Inputs[0].Bip32Derivation = []*psbt.Bip32Derivation{{
		PubKey:    addrKey.SerializedPubKey(),
	...
	require.Equal(t.t, partialSig.PubKey, addrKey.SerializedPubKey())
	require.Greater(t.t, len(partialSig.Signature), 8)

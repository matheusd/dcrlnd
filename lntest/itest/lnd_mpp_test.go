...
func testSendToRouteMultiPath(net *lntest.NetworkHarness, t *harnessTest) {
	...
	openChannel := func(from, to *lntest.HarnessNode, chanSize dcrutil.Amount) {
		ctxt, _ := context.WithTimeout(ctxb, defaultTimeout)
		err := net.SendCoins(ctxt, dcrutil.AtomsPerCoin, from)
		if err != nil {
	...
	buildRoute := func(amt dcrutil.Amount, hops []*lntest.HarnessNode) (
		*lnrpc.Route, error) {
	...
	assertSettledInvoice := func(node *lntest.HarnessNode, rhash []byte,
		...
		for !found {
			...
			for _, inv := range invoicesResp.Invoices {
				...
				// correct.
				if inv.AmtPaidAtoms != int64(paymentAmt) {
					t.Fatalf("incorrect payment amt for "+
						"invoicewant: %d, got %d",
						paymentAmt, inv.AmtPaidAtoms)
				}

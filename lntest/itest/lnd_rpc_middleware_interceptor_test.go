...
func testRPCMiddlewareInterceptor(net *lntest.NetworkHarness, t *harnessTest) {
	...
	aliceOriginalArgs := net.Alice.Cfg.ExtraArgs
	...
	chanAlice := openChannelAndAssert(
		t, net, net.Alice, net.Bob, lntest.OpenChannelParams{
			Amt: 1_234_567,
		},
	)
	defer closeChannelAndAssert(t, net, net.Alice, chanAlice, false)
	...
	t.t.Run("mandatory middleware", func(tt *testing.T) {
		if net.Alice.Cfg.RemoteWallet {
			tt.Skipf("Skipping test because we cannot init a new " +
				"remote wallet when the required middleware is " +
				"offline")
		}
		alice := net.NewNode(t.t, "alice", []string{"--rpcmiddleware.enable"})
		middlewareMandatoryTest(tt, alice, net)
	})

	net.Alice.Cfg.ExtraArgs = aliceOriginalArgs
	err = net.RestartNode(net.Alice, nil)
	require.NoError(t.t, err)
}

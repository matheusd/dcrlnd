...
func (b *Batcher) BatchFund(ctx context.Context,
	...
	for idx, rpcChannel := range req.Channels {
		...
		fundingReq.ChanFunder = chanfunding.NewPsbtAssembler(
			dcrutil.Amount(rpcChannel.LocalFundingAmount), nil,
	...
	feeRateAtomPerKByte := firstReq.FundingFeePerKB
	fundPsbtReq := &walletrpc.FundPsbtRequest{
		Template: &walletrpc.FundPsbtRequest_Raw{
			Raw: txTemplate,
		},
		Fees: &walletrpc.FundPsbtRequest_AtomsPerByte{
			AtomsPerByte: uint64(feeRateAtomPerKByte) / 1000,
		},

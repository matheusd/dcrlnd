...
func (h *testHarness) parseRequest(
	...
	pubKey, err := secp256k1.ParsePubKey(in.NodePubkey)
	if err != nil {
		return nil, err
	}
	...
	return &InitFundingMsg{
		...
		PushAmt: lnwire.NewMAtomsFromAtoms(
			dcrutil.Amount(in.PushAtoms),
		),
		FundingFeePerKB: chainfee.AtomPerKByte(
			in.AtomsPerByte * 1000,
		),
		Private:        in.Private,

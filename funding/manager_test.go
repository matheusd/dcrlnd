...
func testLocalCSVLimit(t *testing.T, aliceMaxCSV, bobRequiredCSV uint16) {
	...
	bob.fundingMgr.cfg.RequiredRemoteDelay = func(_ dcrutil.Amount) uint16 {
...
func TestMaxChannelSizeConfig(t *testing.T) {
	...
	initReq := &InitFundingMsg{
		...
		TargetPubkey:    bob.privKey.PubKey(),
		ChainHash:       fundingNetParams.GenesisHash,
		LocalFundingAmt: MaxDecredFundingAmount,
		PushAmt:         lnwire.NewMAtomsFromAtoms(0),
		Private:         false,
		Updates:         updateChan,
		Err:             errChan,
...
func testUpfrontFailure(t *testing.T, pkscript []byte, expectErr bool) {
	...
	fundingAmt := dcrutil.Amount(500000)
	pushAmt := lnwire.NewMAtomsFromAtoms(dcrutil.Amount(0))

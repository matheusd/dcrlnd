...
type InitFundingMsg struct {
	...
	// FundingFeePerKB is the fee for the funding transaction.
	...
	// MaxValueInFlight is the maximum amount of coins in MilliAtom
...
func (f *Manager) annAfterSixConfs(completeChan *channeldb.OpenChannel,
	...
	if !announceChan {
		...
		log.Infof("Announcing ChannelPoint(%v), short_chan_id=%s",
...
func (f *Manager) handleInitFundingMsg(msg *InitFundingMsg) {
	...
	if commitType.HasAnchors() &&
		commitFeePerKB > f.cfg.MaxAnchorsCommitFeeRate {
		commitFeePerKB = f.cfg.MaxAnchorsCommitFeeRate
	}
	...
	log.Infof("Target commit tx atom/kB for pending_id(%x): %v", chanID,
...
func (f *Manager) pruneZombieReservations() {
	...
	for _, pendingReservations := range f.activeReservations {
		for pendingChanID, resCtx := range pendingReservations {
			locked, _ := resCtx.isLocked()
			if locked {
				continue
			}

			// We don't want to expire PSBT funding reservations.
			// These reservations are always initiated by us and
			// the remote peer is likely going to cancel them after
			// some idle time anyway. So no need for us to also
			// prune them.
	...
	for pendingChanID, resCtx := range zombieReservations {
		err := fmt.Errorf("reservation timed out waiting for peer "+
			"(peer_id:%x, chan_id:%x)", resCtx.peer.IdentityKey().SerializeCompressed(),
			pendingChanID[:])
		log.Warnf(err.Error())
		f.failFundingFlow(resCtx.peer, pendingChanID, err)
	}
...
// reservation.
func (f *Manager) cancelReservationCtx(peerKey *secp256k1.PublicKey,
...
func (f *Manager) deleteReservationCtx(peerKey *secp256k1.PublicKey,
	pendingChanID [32]byte) {
...
func (f *Manager) getReservationCtx(peerKey *secp256k1.PublicKey,

...
func (d *AuthenticatedGossiper) resendAnnounceSignatures() error {
	...
	for peer, msgsToResend := range peerMsgsToResend {
		pubKey, err := secp256k1.ParsePubKey(peer[:])
		if err != nil {
			return err
		}
...
func (d *AuthenticatedGossiper) sendAnnSigReliably(
	...
	// we do not succeed in sending it to the peer, we'll fetch it from the
	// DB next time we start, and retry.
...
func (d *AuthenticatedGossiper) updateChannel(info *channeldb.ChannelEdgeInfo,
	...
	*lnwire.ChannelUpdate, error) {
	...
	chanUpdate := &lnwire.ChannelUpdate{
		...
		HtlcMaximumMAtoms: edge.MaxHTLC,

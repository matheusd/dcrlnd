...
func createAnnouncements(blockHeight uint32, key1, key2 *secp256k1.PrivateKey) (*annBatch, error) {
	var err error
	var batch annBatch
	...
	batch.remoteProofAnn = &lnwire.AnnounceSignatures{
		...
		DecredSignature: batch.chanAnn.DecredSig2,
	}
	...
	batch.localProofAnn = &lnwire.AnnounceSignatures{
		...
		DecredSignature: batch.chanAnn.DecredSig1,
	}
...
func createUpdateAnnouncement(blockHeight uint32,
	...
	htlcMinMatoms := lnwire.MilliAtom(prand.Int63())
	a := &lnwire.ChannelUpdate{
		...
		ChannelFlags:      flags,
		HtlcMinimumMAtoms: htlcMinMatoms,

		// Since the max HTLC must be greater than the min HTLC to pass
		// channel update validation, set it to double the min htlc.
		HtlcMaximumMAtoms: 2 * htlcMinMatoms,
...
func createChannelAnnouncement(blockHeight uint32, key1, key2 *secp256k1.PrivateKey,
	extraBytes ...[]byte) (*lnwire.ChannelAnnouncement, error) {
...
func createTestCtx(startHeight uint32) (*testCtx, func(), error) {
	...
	gossiper := New(Config{
		...
		NotifyWhenOnline: func(target [33]byte,
			...
			pk, _ := secp256k1.ParsePubKey(target[:])
...
func TestSignatureAnnouncementRetryAtStartup(t *testing.T) {
	...
	// Since the reliable send to the remote peer of the local channel
	// proof requires a notification when the peer comes online, we'll
	// capture the channel through which it gets sent to control exactly
	// when to dispatch it.
...
func TestOptionalFieldsChannelUpdateValidation(t *testing.T) {
	...
	chanUpdAnn.HtlcMinimumMAtoms = 5000
	chanUpdAnn.HtlcMaximumMAtoms = 4000
	...
	chanUpdAnn.HtlcMinimumMAtoms = 0
	chanUpdAnn.HtlcMaximumMAtoms = 0
	if err := signUpdate(remoteKeyPriv1, chanUpdAnn); err != nil {
		t.Fatalf("unable to sign channel update: %v", err)
	}
...
func TestProcessChannelAnnouncementOptionalMsgFields(t *testing.T) {
	...
	assertOptionalMsgFields := func(chanID lnwire.ShortChannelID,
		capacity dcrutil.Amount, channelPoint wire.OutPoint) {

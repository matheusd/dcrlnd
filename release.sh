...
for i in $SYS; do
    ...
    env GOOS=$OS GOARCH=$ARCH GOARM=$ARM go build -v -ldflags "$COMMITFLAGS" -tags="autopilot signrpc walletrpc chainrpc invoicesrpc routerrpc" github.com/decred/dcrlnd/cmd/dcrlnd
    env GOOS=$OS GOARCH=$ARCH GOARM=$ARM go build -v -ldflags "$COMMITFLAGS" -tags="autopilotrpc invoicesrpc walletrpc routerrpc" github.com/decred/dcrlnd/cmd/dcrlncli
    cd ..

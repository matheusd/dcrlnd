...
// WitnessStackToSigScript converts a witness stack, which is essentially just
// an array of byte slices, to the equivalent signature script such that each
// element in the witness stack is a data push.
...
func SpendMultiSig(witnessScript, pubA []byte, sigA Signature,
	...
	// When initially generating the witnessScript, we sorted the
	// serialized public keys in descending order. So we do a quick
	// comparison in order ensure the signatures appear on the Script
	// Virtual Machine stack in the correct order.
	if bytes.Compare(pubA, pubB) == 1 {
		witness[0] = append(sigB.Serialize(), byte(txscript.SigHashAll))
		witness[1] = append(sigA.Serialize(), byte(txscript.SigHashAll))
	} else {
		witness[0] = append(sigA.Serialize(), byte(txscript.SigHashAll))
...
// ReplaceReceiverHtlcSpendRedeemPreimage replaces the pregimage in a sigScript
// generated by receiverHtlcSpendRedeemPreimage with the provided preimage. This
// will generate an undefined result script if the sigScript was *not* generated
// by the provided function (ie: this must be the sigScript of a
// SignedSuccessTx, used to redeem an HTLC).
//
...
func ReplaceReceiverHtlcSpendRedeemPreimage(sigScript, preimage []byte) ([]byte, error) {
	...
	if len(sigScriptPushes) != 4 {
		return nil, fmt.Errorf("the provided sigScript does not have 4 elements")
	}

	return txscript.NewScriptBuilder().
		AddData(sigScriptPushes[0]). // sender sig
		AddData(sigScriptPushes[1]). // receiver sig
		AddData(preimage).           // preimage
		AddData(sigScriptPushes[3]). // redeem script
		Script()
}

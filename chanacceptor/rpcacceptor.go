...
func (r *RPCAcceptor) receiveResponses(errChan chan error,
	...
	for {
		...
		openChanResp := &lnrpc.ChannelAcceptResponse{
			...
			ReserveAtoms:      resp.ReserveAtoms,
			InFlightMaxMatoms: resp.InFlightMaxMatoms,
	...
	for {
		...
		case resp := <-responses:
			...
			requestInfo.response <- NewChannelAcceptResponse(
				...
				dcrutil.Amount(resp.ReserveAtoms),
				lnwire.MilliAtom(resp.InFlightMaxMatoms),
				lnwire.MilliAtom(resp.MinHtlcIn),
			)
...
func (r *RPCAcceptor) validateAcceptorResponse(dustLimit dcrutil.Amount,
	...
	// required by BOLT 2.
	reserveAtoms := dcrutil.Amount(req.ReserveAtoms)
	if reserveAtoms != 0 && reserveAtoms < dustLimit {
		log.Errorf("Remote reserve: %v atoms for channel: %v must be "+
			"at least equal to proposed dust limit: %v",
			req.ReserveAtoms, channelStr, dustLimit)

...
func setup(t *testing.T, initialChans []Channel) (*testContext, func()) {
	...
	var availableFunds dcrutil.Amount = 10 * dcrutil.AtomsPerCoin
...
func TestAgentChannelFailureSignal(t *testing.T) {
	...
	respondMoreChans(t, testCtx, moreChansResp{1, 5 * dcrutil.AtomsPerCoin})
...
func TestAgentBalanceUpdate(t *testing.T) {
	...
	testCtx.Lock()
	testCtx.walletBalance += dcrutil.AtomsPerCoin * 5
	testCtx.Unlock()
...
func TestAgentPendingChannelState(t *testing.T) {
	...
	// heuristic.
	testCtx.Lock()
	testCtx.walletBalance += 0.4 * dcrutil.AtomsPerCoin
	testCtx.Unlock()
	...
	// channels.
	select {
	case testCtx.constraints.moreChansResps <- moreChansResp{1, dcrutil.AtomsPerCoin}:
	case <-time.After(time.Second * 10):
		t.Fatalf("need more chans wasn't queried in time")
	}
...
func TestAgentSkipPendingConns(t *testing.T) {
	...
	testCtx.agent.cfg.ConnectToPeer = func(*secp256k1.PublicKey, []net.Addr) (bool, error) {
...
func respondWithScores(t *testing.T, testCtx *testContext,
	channelBudget dcrutil.Amount, existingChans, newChans int,
...
func checkChannelOpens(t *testing.T, testCtx *testContext,
	allocation dcrutil.Amount, numChans int) []NodeID {

	var nodes []NodeID

	// The agent should attempt to open channels, totaling what we expect.
	var totalAllocation dcrutil.Amount
	chanController := testCtx.chanController.(*mockChanController)
	for i := 0; i < numChans; i++ {
		select {
		case openChan := <-chanController.openChanSignals:
...
func TestAgentChannelSizeAllocation(t *testing.T) {
	...
	var channelBudget dcrutil.Amount = 5 * dcrutil.AtomsPerCoin
	...
	// channels.
	expectedAllocation := testCtx.constraints.MaxChanSize() * dcrutil.Amount(numNewChannels)
	...
	channelBudget = dcrutil.AtomsPerCoin * 3 / 2
	...
	// our budget.
	channelBudget = dcrutil.AtomsPerCoin * 5
	numNewChannels = 2
	respondWithScores(
		t, testCtx, channelBudget, numExistingChannels,
		numNewChannels, nodeScores,

...
func toRPCPairData(data *routing.TimedPairResult) *PairData {
	rpcData := PairData{
		FailAmtAtoms:     int64(data.FailAmt.ToAtoms()),
		FailAmtMAtoms:    int64(data.FailAmt),
		SuccessAmtAtoms:  int64(data.SuccessAmt.ToAtoms()),
		SuccessAmtMAtoms: int64(data.SuccessAmt),
	}
...
func toPairSnapshot(pairResult *PairHistory) (*routing.MissionControlPairSnapshot,
	...
	failAmt, failTime, err := getPair(
		lnwire.MilliAtom(pairResult.History.FailAmtMAtoms),
		dcrutil.Amount(pairResult.History.FailAmtAtoms),
	...
	successAmt, successTime, err := getPair(
		lnwire.MilliAtom(pairResult.History.SuccessAmtMAtoms),
		dcrutil.Amount(pairResult.History.SuccessAmtAtoms),
...
func getPair(amtMAtoms lnwire.MilliAtom, amtAtoms dcrutil.Amount,
	timestamp int64) (lnwire.MilliAtom, time.Time, error) {

	amt, err := getMsatPairValue(amtMAtoms, amtAtoms)
	if err != nil {
...
func getMsatPairValue(msatValue lnwire.MilliAtom,
	satValue dcrutil.Amount) (lnwire.MilliAtom, error) {

	// If our msat value converted to sats equals our sat value, we just
	// return the msat value, since the values are the same.
	if msatValue.ToAtoms() == satValue {
	...
	if msatValue == 0 {
		return lnwire.MilliAtom(satValue * 1000), nil
	}
...
func (s *Server) QueryProbability(ctx context.Context,
	...
	amt := lnwire.MilliAtom(req.AmtMAtoms)
...
func (s *Server) BuildRoute(ctx context.Context,
	...
	if req.AmtMAtoms != 0 {
		rpcAmt := lnwire.MilliAtom(req.AmtMAtoms)
		amt = &rpcAmt
	}

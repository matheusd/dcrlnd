...
func testQueryRoutes(t *testing.T, useMissionControl bool, useMAtoms bool) {
	...
	amtAtoms := int64(100000)
	if useMAtoms {
		request.AmtMAtoms = amtAtoms * 1000
...
func testUnmarshalMPP(t *testing.T, test unmarshalMPPTest) {
	...
	case valid:
		...
		if int64(mpp.TotalMAtoms()) != test.mpp.TotalAmtMAtoms {

...
func TestSwitchForwardFailAfterFullAdd(t *testing.T) {
	...
	// Create request which should be forwarded from Alice channel link to
	// bob channel link.
	preimage := [chainhash.HashSize]byte{1}
	rhash := sha256.Sum256(preimage[:])
	ogPacket := &htlcPacket{
		incomingChanID: aliceChannelLink.ShortChanID(),
		incomingHTLCID: 0,
...
func TestInvalidFailure(t *testing.T) {
	...
	rhash := lntypes.Hash(sha256.Sum256(preimage[:]))
	update := &lnwire.UpdateAddHTLC{
		PaymentHash: rhash,
		Amount:      1,
	}
...
func TestSwitchDustForwarding(t *testing.T) {
	...
	// - Alice has a dust limit of 6030 atoms with Bob
	// - Bob has a dust limit of 12060 atoms with Alice
	// - Bob has a dust limit of 6030 atoms with Carol
	// - Carol has a dust limit of 12060 atoms with Bob
	channels, cleanUp, _, err := createClusterChannels(
		dcrutil.AtomsPerCoin, dcrutil.AtomsPerCoin,
	)
	...
	amt := lnwire.NewMAtomsFromAtoms(1677)
	...
	// false.
	err = n.bobServer.htlcSwitch.SendHTLC(
		aliceBobFirstHop, uint64(150), nondustHtlc,
	)
	require.NoError(t, err)
...
func sendDustHtlcs(t *testing.T, n *threeHopNetwork, alice bool,
	amt lnwire.MilliAtom, sid lnwire.ShortChannelID) {
	...
	for i := 0; i < numHTLCs; i++ {
		...
		require.NoError(t, err, "at i %d", i)
		attemptID++
	}
}
...
func TestSwitchMailboxDust(t *testing.T) {
	...
	require.NoError(t, err)
	rhash := sha256.Sum256(preimage[:])
	amt := lnwire.NewMAtomsFromAtoms(350)
	addMsg := &lnwire.UpdateAddHTLC{
		PaymentHash: rhash,
		Amount:      amt,
		ChanID:      chanID1,

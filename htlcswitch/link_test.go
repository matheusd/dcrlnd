...
func TestChannelLinkCancelFullCommitment(t *testing.T) {
	...
	amt := lnwire.NewMAtomsFromAtoms(20000)
...
func TestLinkForwardFeePolicyMismatch(t *testing.T) {
	...
	// current default fee of 1 Atom, if we just send a single DCR over in
	// an HTLC, it should be rejected.
	amountNoFee := lnwire.NewMAtomsFromAtoms(dcrutil.AtomsPerCoin)
...
func TestLinkForwardMinHTLCPolicyMismatch(t *testing.T) {
	...
	// for the three-hop-network is 5 Atoms. So in order to trigger this
	// failure mode, we'll create an HTLC with 1 atom.
	amountNoFee := lnwire.NewMAtomsFromAtoms(1)
...
func TestLinkForwardMaxHTLCPolicyMismatch(t *testing.T) {
	...
	channels, cleanUp, _, err := createClusterChannels(
		dcrutil.AtomsPerCoin*5, dcrutil.AtomsPerCoin*5,
	)
	if err != nil {
		t.Fatalf("unable to create channel: %v", err)
	}
	...
	// In order to trigger this failure mode, we'll update our policy to
	// have a new max HTLC of 10 atoms.
...
func TestUpdateForwardingPolicy(t *testing.T) {
	...
	amountNoFee := lnwire.NewMAtomsFromAtoms(10)
	...
	newPolicy.BaseFee = lnwire.NewMAtomsFromAtoms(1000)
...
func TestChannelLinkMultiHopInsufficientPayment(t *testing.T) {
	...
	// rejected.
	amount := lnwire.NewMAtomsFromAtoms(4 * dcrutil.AtomsPerCoin)
	htlcAmt, totalTimelock, hops := generateHops(amount, testStartingHeight,
		n.firstBobChannelLink, n.carolChannelLink)
...
func TestChannelLinkMultiHopDecodeError(t *testing.T) {
	...
	n.carolChannelLink.cfg.ExtractErrorEncrypter = func(
		*secp256k1.PublicKey) (hop.ErrorEncrypter, lnwire.FailCode) {
...
func handleStateUpdate(link *channelLink,
	...
	err := remoteChannel.ReceiveNewCommitment(
		commitSig.CommitSig, commitSig.HtlcSigs, commitSig.PtlcSigs)
	if err != nil {
		return err
	}
	...
	remoteSig, remoteHtlcSigs, remotePtlcSigs, _, err := remoteChannel.SignNextCommitment()
	if err != nil {
		return err
	}
...
func TestChannelLinkTrimCircuitsRemoteCommit(t *testing.T) {
	...
	case aliceMsg := <-alice.msgs:
		...
		err := bobChan.ReceiveNewCommitment(sig.CommitSig, sig.HtlcSigs, sig.PtlcSigs)
		if err != nil {
...
func TestChannelLinkAcceptOverpay(t *testing.T) {
	...
	// We'll request a route to send 10k atoms via Alice -> Bob ->
	// Carol.
	amount := lnwire.NewMAtomsFromAtoms(dcrutil.AtomsPerCoin)
	htlcAmt, totalTimelock, hops := generateHops(
		amount, testStartingHeight,
...
func generateHtlcAndInvoice(t *testing.T,
	...
	hops := []*hop.Payload{
		hop.NewLegacyPayload(&sphinx.HopData{
			Realm:         [1]byte{2}, // hop.DecredNetwork
...
// TestChannelLinkNoMoreUpdates tests that we won't send a new commitment when
// there are no new updates to sign.
...
func TestChannelLinkNoEmptySig(t *testing.T) {
	...
	err = bobChannel.ReceiveNewCommitment(
		commitSigAlice.CommitSig, commitSigAlice.HtlcSigs, commitSigAlice.PtlcSigs,
	)
	if err != nil {
...
type hodlInvoiceTestCtx struct {
	n                   *twoHopNetwork
	startBandwidthAlice lnwire.MilliAtom
	startBandwidthBob   lnwire.MilliAtom
	hash                lntypes.Hash

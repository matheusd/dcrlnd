	...
	for j, node := range nodes {
		...
		for _, msg := range announcements {
			...
			case *lnwire.ChannelUpdate:
				...
				maxPendingMatoms := alice.fundingMgr.cfg.RequiredRemoteMaxValue(
					capacity,
				)
				if maxPendingMatoms != m.HtlcMaximumMAtoms {
					t.Fatalf("expected ChannelUpdate to "+
...
func TestFundingManagerRejectInvalidMaxValueInFlight(t *testing.T) {
	...
	alice.fundingMgr.cfg.RequiredRemoteMaxValue = func(
		_ dcrutil.Amount) lnwire.MilliAtom {
		return lnwire.NewMAtomsFromAtoms(capacity) + 100
	}

...
type CommitmentKeyRing struct {
	...
	ToLocalKey *secp256k1.PublicKey
	...
	ToRemoteKey *secp256k1.PublicKey
...
// points and commitment point. The keys are derived differently depending on
// the type of channel, and whether the commitment transaction is ours or the
// remote peer's.
func DeriveCommitmentKeys(commitPoint *secp256k1.PublicKey,
	...
	var (
		toLocalBasePoint    *secp256k1.PublicKey
		toRemoteBasePoint   *secp256k1.PublicKey
...
func CommitScriptToSelf(chanType channeldb.ChannelType, initiator bool,
	selfKey, revokeKey *secp256k1.PublicKey, csvDelay, leaseExpiry uint32) (
	*ScriptInfo, error) {
	...
	toLocalScriptHash, err := input.ScriptHashPkScript(toLocalRedeemScript)
	if err != nil {
		return nil, err
	}
...
func CommitScriptToRemote(chanType channeldb.ChannelType, initiator bool,
	key *secp256k1.PublicKey, leaseExpiry uint32) (*ScriptInfo, uint32, error) {
	...
	case chanType.HasLeaseExpiration() && !initiator:
		...
		p2sh, err := input.ScriptHashPkScript(script)
		if err != nil {
			return nil, 0, err
		}
	...
	default:
		// Otherwise the to_remote will be a simple p2pkh.
		p2pkh, err := input.CommitScriptUnencumbered(key)
		if err != nil {
			return nil, 0, err
		}

		// Since this is a regular P2PKH, the WitnessScipt and PkScript
...
func SecondLevelHtlcScript(chanType channeldb.ChannelType, initiator bool,
	revocationKey, delayKey *secp256k1.PublicKey,
	...
	// additional CLTV requirement in addition to the usual CSV requirement.
	case initiator && chanType.HasLeaseExpiration():
		redeemScript, err = input.LeaseSecondLevelHtlcScript(
			revocationKey, delayKey, csvDelay, leaseExpiry,
		)

	default:
		redeemScript, err = input.SecondLevelHtlcScript(
	...
	pkScript, err := input.ScriptHashPkScript(redeemScript)
	if err != nil {
		return nil, err
	}
...
func CommitScriptAnchors(localChanCfg,
	...
	*ScriptInfo, error) {
	...
	anchorScript := func(key *secp256k1.PublicKey) (*ScriptInfo, error) {
		script, err := input.CommitScriptAnchor(key)
		if err != nil {
			return nil, err
		}

		scriptHash, err := input.ScriptHashPkScript(script)
		if err != nil {
			return nil, err
		}
...
type CommitmentBuilder struct {
	...
	// netParams are the chain parameters for which commitments are built.
	netParams *chaincfg.Params
}

// NewCommitmentBuilder creates a new CommitmentBuilder from chanState.
func NewCommitmentBuilder(chanState *channeldb.OpenChannel, netParams *chaincfg.Params) *CommitmentBuilder {
...
func (cb *CommitmentBuilder) createUnsignedCommitmentTx(ourBalance,
	...
	switch {
	case cb.chanState.IsInitiator && commitFee > ourBalance.ToAtoms():
		ourBalance = 0

	case cb.chanState.IsInitiator:
		ourBalance -= commitFeeMAtoms

	case !cb.chanState.IsInitiator && commitFee > theirBalance.ToAtoms():
		theirBalance = 0
	...
	if isOurs {
		commitTx, err = CreateCommitTx(
			...
			ourBalance.ToAtoms(), theirBalance.ToAtoms(),
		...
		commitTx, err = CreateCommitTx(
			...
			theirBalance.ToAtoms(), ourBalance.ToAtoms(),
			numHTLCs, !cb.chanState.IsInitiator, leaseExpiry,
		)
	}
	if err != nil {
...
func CreateCommitTx(chanType channeldb.ChannelType,
	...
	amountToLocal, amountToRemote dcrutil.Amount,
...
func CoopCloseBalance(chanType channeldb.ChannelType, isInitiator bool,
	coopCloseFee dcrutil.Amount, localCommit channeldb.ChannelCommitment) (
	dcrutil.Amount, dcrutil.Amount, error) {

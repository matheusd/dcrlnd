...
type ErrCommitSyncLocalDataLoss struct {
	...
	CommitPoint *secp256k1.PublicKey
}
...
func locateOutputIndex(p *PaymentDescriptor, tx *wire.MsgTx, ourCommit bool,
	...
	for i, txOut := range tx.TxOut {
		if bytes.Equal(txOut.PkScript, pkScript) &&
			txOut.Value == int64(p.Amount.ToAtoms()) {
...
func (lc *LightningChannel) diskHtlcToPayDesc(feeRate chainfee.AtomPerKByte,
...
func (lc *LightningChannel) extractPayDescs(commitHeight uint64,
	feeRate chainfee.AtomPerKByte, htlcs []channeldb.HTLC, localCommitKeys,
...
func (lc *LightningChannel) diskCommitToMemCommit(isLocal bool,
	...
	remoteCommitPoint *secp256k1.PublicKey) (*commitment, error) {
	...
	incomingHtlcs, outgoingHtlcs, err := lc.extractPayDescs(
		...
		chainfee.AtomPerKByte(diskCommit.FeePerKB),
	...
	commit := &commitment{
		...
		feePerKB:          chainfee.AtomPerKByte(diskCommit.FeePerKB),
...
type LightningChannel struct {
	// input.Signer is the main signer instances that will be responsible for
	...
	LocalFundingKey *secp256k1.PublicKey
...
// createSignDesc derives the input.SignDescriptor for commitment transactions from
...
func (lc *LightningChannel) createSignDesc() error {
	...
	fundingPkScript, err := input.ScriptHashPkScript(multiSigScript)
	if err != nil {
		return err
	}
...
func (lc *LightningChannel) logUpdateToPayDesc(logUpdate *channeldb.LogUpdate,
	...
	feeRate chainfee.AtomPerKByte, remoteCommitKeys *CommitmentKeyRing,
	remoteDustLimit dcrutil.Amount) (*PaymentDescriptor, error) {
	...
	// For HTLCs we're offered we'll fetch the original offered HTLC
	...
	case *lnwire.UpdateFee:
		pd = &PaymentDescriptor{
			LogIndex: logUpdate.LogIndex,
			Amount: lnwire.NewMAtomsFromAtoms(
				dcrutil.Amount(wireMsg.FeePerKB),
			),
			EntryType:                FeeUpdate,
			addCommitHeightRemote:    commitHeight,
			removeCommitHeightRemote: commitHeight,
...
type BreachRetribution struct {
	...
	// LocalOutputSignDesc is a input.SignDescriptor which is capable of
...
func NewBreachRetribution(chanState *channeldb.OpenChannel, stateNum uint64,
	...
	commitmentSecret, commitmentPoint := secp256k1.PrivKeyFromBytes(
		revocationPreimage[:])
	...
	theirWitnessHash, err := input.ScriptHashPkScript(theirPkScript)
	if err != nil {
		return nil, err
	}
	...
	// Compute the balances in atoms.
	ourAmt := revokedSnapshot.LocalBalance.ToAtoms()
	theirAmt := revokedSnapshot.RemoteBalance.ToAtoms()
	...
	for _, htlc := range revokedSnapshot.Htlcs {
		...
		htlcPkScript, err := input.ScriptHashPkScript(htlcWitnessScript)
		if err != nil {
			return nil, err
		}
...
func htlcTimeoutFee(feePerKB chainfee.AtomPerKByte) dcrutil.Amount {
	return feePerKB.FeeForSize(input.HTLCTimeoutTxSize)
}

// htlcSuccessFee returns the fee in atoms required for an HTLC success
// transaction based on the current fee rate.
func htlcSuccessFee(feePerKB chainfee.AtomPerKByte) dcrutil.Amount {
...
func htlcIsDust(incoming, ourCommit bool, feePerKB chainfee.AtomPerKByte,
...
func (lc *LightningChannel) fetchCommitmentView(remoteChain bool,
	...
	commitTx, err := lc.commitBuilder.createUnsignedCommitmentTx(
		ourBalance, theirBalance, !remoteChain, feePerKB, nextHeight,
...
func (lc *LightningChannel) evaluateHTLCView(view *htlcView, ourBalance,
	...
	for _, entry := range view.ourUpdates {
		...
		// Process fee updates, updating the current feePerKB.
		case FeeUpdate:
			processFeeUpdate(
...
func processAddEntry(htlc *PaymentDescriptor, ourBalance, theirBalance *lnwire.MilliAtom,
...
func processFeeUpdate(feeUpdate *PaymentDescriptor, nextHeight uint64,
	...
	view.feePerKB = chainfee.AtomPerKByte(feeUpdate.Amount.ToAtoms())
...
func genRemoteHtlcSigJobs(keyRing *CommitmentKeyRing,
	...
	for _, htlc := range remoteCommitView.incomingHTLCs {
		if htlcIsDust(true, false, feePerKB, htlc.Amount.ToAtoms(),
			dustLimit) {
			continue
		}
	...
	for _, htlc := range remoteCommitView.outgoingHTLCs {
		if htlcIsDust(false, false, feePerKB, htlc.Amount.ToAtoms(),
			dustLimit) {
			continue
		}
	...
	for e := lc.localUpdateLog.Front(); e != nil; e = e.Next() {
		...
		case FeeUpdate:
			...
			logUpdate.UpdateMsg = &lnwire.UpdateFee{
				ChanID:   chanID,
				FeePerKB: uint32(pd.Amount.ToAtoms()),
			}
		}
...
func (lc *LightningChannel) validateCommitmentSanity(theirLogCounter,
	...
	validateUpdates := func(updates []*PaymentDescriptor,
		...
		var amtInFlight lnwire.MilliAtom
...
func (lc *LightningChannel) computeView(view *htlcView, remoteChain bool,
	...
	*htlcView, error) {
	...
	// rate.
	view.feePerKB = commitChain.tip().feePerKB
...
// new commitment.
func (lc *LightningChannel) NextRevocationKey() (*secp256k1.PublicKey, error) {
	lc.RLock()
	defer lc.RUnlock()
...
// any new states.
func (lc *LightningChannel) InitNextRevocation(revKey *secp256k1.PublicKey) error {
	lc.Lock()
	defer lc.Unlock()
...
func (lc *LightningChannel) getSignedCommitTx() (*wire.MsgTx, error) {
	...
	// Finally, convert the witness stack to a signature script and set it.
	sigScript, err := input.WitnessStackToSigScript(witness)
	if err != nil {
		return nil, err
	}
	commitTx.TxIn[0].SignatureScript = sigScript
...
// the remote party's commitment transaction, the necessary a input.SignDescriptor
...
func NewUnilateralCloseSummary(chanState *channeldb.OpenChannel, signer input.Signer,
	...
	remoteCommit channeldb.ChannelCommitment,
	commitPoint *secp256k1.PublicKey) (*UnilateralCloseSummary, error) {
	...
	closeSummary := channeldb.ChannelCloseSummary{
		...
		SettledBalance:          dcrutil.Amount(localBalance),
...
type IncomingHtlcResolution struct {
	...
	// to fully sweep the HTLC. The input.SignDescriptor below should be used to
...
func newOutgoingHtlcResolution(signer input.Signer,
	...
	if !localCommit {
		...
		htlcReceiverScript, err := input.ReceiverHTLCScript(
			htlc.RefundTimeout, keyRing.LocalHtlcKey,
			keyRing.RemoteHtlcKey, keyRing.RevocationKey,
			htlc.RHash[:],
		)
		...
		htlcScriptHash, err := input.ScriptHashPkScript(htlcReceiverScript)
		if err != nil {
			return nil, err
		}
	...
	htlcScriptHash, err := input.ScriptHashPkScript(htlcSweepScript)
	if err != nil {
		return nil, err
	}
...
func newIncomingHtlcResolution(signer input.Signer, localChanCfg *channeldb.ChannelConfig,
	...
	if !localCommit {
		...
		htlcScriptHash, err := input.ScriptHashPkScript(htlcSenderScript)
		if err != nil {
			return nil, err
		}
	...
	successTx.TxIn[0].SignatureScript, err = input.WitnessStackToSigScript(successWitness)
	if err != nil {
		return nil, err
	}
	...
	for _, htlc := range htlcs {
		...
		if htlc.Incoming {
			...
			ihr, err := newIncomingHtlcResolution(
				...
				feePerKB, uint32(csvDelay), ourCommit,
			)
			if err != nil {
				return nil, err
			}
...
func NewLocalForceCloseSummary(chanState *channeldb.OpenChannel, signer input.Signer,
	...
	*LocalForceCloseSummary, error) {
	...
	payToUsScriptHash, err := input.ScriptHashPkScript(selfScript)
	if err != nil {
		return nil, err
	}
		...
		commitResolution = &CommitOutputResolution{
			...
			SelfOutputSignDesc: input.SignDescriptor{
				...
				Output: &wire.TxOut{
					...
					Value:    int64(localBalance.ToAtoms()),
				},
				HashType: txscript.SigHashAll,
			},
...
func (lc *LightningChannel) AvailableBalance() lnwire.MilliAtom {
	lc.RLock()
	defer lc.RUnlock()
...
func (lc *LightningChannel) availableBalance() (lnwire.MilliAtom, int64) {
	...
	ourLocalCommitBalance, commitSize := lc.availableCommitmentBalance(
		htlcView, false,
	)
	...
	if ourRemoteCommitBalance < ourLocalCommitBalance {
		return ourRemoteCommitBalance, commitSize
	}

	return ourLocalCommitBalance, commitSize
}
...
func (lc *LightningChannel) availableCommitmentBalance(view *htlcView,
	remoteChain bool) (lnwire.MilliAtom, int64) {
	...
	feePerKB := filteredView.feePerKB
	htlcCommitFee := lnwire.NewMAtomsFromAtoms(
		feePerKB.FeeForSize(commitSize + input.HTLCOutputSize),
	)
...
func (lc *LightningChannel) validateFeeRate(feePerKB chainfee.AtomPerKByte) error {
...
// the remote.
func (lc *LightningChannel) UpdateFee(feePerKB chainfee.AtomPerKByte) error {
	lc.Lock()
	defer lc.Unlock()
	...
	pd := &PaymentDescriptor{
		LogIndex:  lc.localUpdateLog.logIndex,
		Amount:    lnwire.NewMAtomsFromAtoms(dcrutil.Amount(feePerKB)),
		EntryType: FeeUpdate,
	}
...
func (lc *LightningChannel) ReceiveUpdateFee(feePerKB chainfee.AtomPerKByte) error {
	lc.Lock()
	defer lc.Unlock()
...
func CreateCooperativeCloseTx(fundingTxIn wire.TxIn,
	localDust, remoteDust, ourBalance, theirBalance dcrutil.Amount,
...
// units of atom-per-kb.
func (lc *LightningChannel) CommitFeeRate() chainfee.AtomPerKByte {
	lc.RLock()
	defer lc.RUnlock()

	return chainfee.AtomPerKByte(lc.channelState.LocalCommitment.FeePerKB)
}
...
// the channel.
func (lc *LightningChannel) MarkDataLoss(commitPoint *secp256k1.PublicKey) error {
	lc.Lock()
	defer lc.Unlock()
...
func (lc *LightningChannel) LocalChanReserve() dcrutil.Amount {
...
func (lc *LightningChannel) FwdMinHtlc() lnwire.MilliAtom {

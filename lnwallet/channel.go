...
func (lc *LightningChannel) MayAddOutgoingHtlc(amt lnwire.MilliAtom) error {
	lc.Lock()
	defer lc.Unlock()

	var mockHtlcAmt lnwire.MilliAtom
	switch {
...
func newOutgoingHtlcResolution(signer input.Signer,
	...
	htlcSweepScriptHash, err := input.ScriptHashPkScript(htlcSweepScript)
	if err != nil {
		return nil, err
	}
...
func (lc *LightningChannel) CreateCloseProposal(proposedFee dcrutil.Amount,
	...
	// negative output.
	if err := standalone.CheckTransactionSanity(closeTx, uint64(lc.netParams.MaxTxSize)); err != nil {
...
func (lc *LightningChannel) availableCommitmentBalance(view *htlcView,
	...
	htlcFee := lnwire.NewMAtomsFromAtoms(
		HtlcTimeoutFee(lc.channelState.ChanType, feePerKB),
	)
	...
	if remoteChain {
		...
		htlcFee = lnwire.NewMAtomsFromAtoms(
			HtlcSuccessFee(lc.channelState.ChanType, feePerKB),
		)
	}
...
func (lc *LightningChannel) IdealCommitFeeRate(netFeeRate, minRelayFeeRate,
	maxAnchorCommitFeeRate chainfee.AtomPerKByte,
	maxFeeAlloc float64) chainfee.AtomPerKByte {
	...
	var commitFeeRate chainfee.AtomPerKByte
	...
	case true:
		commitFeeRate = chainfee.AtomPerKByte(
			math.Min(
				float64(netFeeRate),
	...
	// Decred network has a higher relay floor, so impose it here.
	if commitFeeRate < chainfee.FeePerKBFloor {
		commitFeeRate = chainfee.FeePerKBFloor
	}

...
func (l *LightningWallet) handleFundingReserveRequest(req *InitFundingReserveMsg) {
	...
	if req.LocalFundingAmt != 0 {
		...
		selected, err = l.selectCoinsAndChange(
			req.FundingFeePerKB, req.LocalFundingAmt, req.MinConfs,
		)
		if err != nil {
			req.err <- err
			req.resp <- nil
...
func (l *LightningWallet) initOurContribution(reservation *ChannelReservation,
	selected *coinSelection, nodeAddr net.Addr, nodeID *secp256k1.PublicKey) error {
...
func CreateCommitmentTxns(localBalance, remoteBalance dcrutil.Amount,
	...
	fundingTxIn wire.TxIn, chainParams *chaincfg.Params) (*wire.MsgTx, *wire.MsgTx, error) {
...
func (l *LightningWallet) handleContributionMsg(req *addContributionMsg) {
	...
	pendingReservation.fundingTx.Version = 1
...
func (l *LightningWallet) handleFundingCounterPartySigs(msg *addCounterPartySigsMsg) {
	...
	sigHash, err := txscript.CalcSignatureHash(witnessScript,
		txscript.SigHashAll, commitTx, 0, nil)
	if err != nil {
		msg.err <- err
		msg.completeChan <- nil
		return
...
func (l *LightningWallet) handleSingleFunderSigs(req *addSingleFunderSigsMsg) {
	...
	sigHash, err := txscript.CalcSignatureHash(witnessScript,
		txscript.SigHashAll, ourCommitTx, 0, nil)
	if err != nil {
		req.err <- err
		req.completeChan <- nil
		return
...
// outputs which sum to at least 'amt' amount of satoshis. If necessary, a
// change address will also be generated. If coin selection is
...
func (l *LightningWallet) selectCoinsAndChange(feeRate AtomPerKByte,
	amt dcrutil.Amount, minConfs int32) (*coinSelection, error) {
	...
	for i, coin := range selectedCoins {
		...
		inputs[i] = wire.NewTxIn(outpoint, int64(coin.Value), nil)
	}

...
func (l *LightningWallet) handleFundingReserveRequest(req *InitFundingReserveMsg) {
	...
	if req.ChanFunder == nil {
		cfg := chanfunding.WalletConfig{
			...
			CoinLocker:       l,
			Signer:           l.Cfg.Signer,
			DustLimit:        DustLimitForSize(input.P2SHPkScriptSize),
		}
...
//
// NOTE: This method should only be run with the CoinSelectLock held.
func (l *LightningWallet) CheckReservedValueTx(tx *wire.MsgTx) (dcrutil.Amount,
	error) {
...
func validateUpfrontShutdown(shutdown lnwire.DeliveryAddress,
	...
	scriptClass := stdscript.DetermineScriptType(0, shutdown)

	switch scriptClass {
	case stdscript.STPubKeyHashEcdsaSecp256k1,
		stdscript.STScriptHash:
		// The above two types are permitted according to BOLT#02.

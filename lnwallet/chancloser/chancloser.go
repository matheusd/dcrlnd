...
type ChanCloser struct {
	...
	// party.  We'll use this as a pivot point to ratchet our next offer
	// up, down, or simply accept the remote party's prior offer.
...
func NewChanCloser(cfg ChanCloseCfg, deliveryScript []byte,
	idealFeePerKB chainfee.AtomPerKByte, negotiationHeight uint32,
	...
	idealFeeSat := cfg.Channel.CalcFee(idealFeePerKB)
...
func (c *ChanCloser) proposeCloseSigned(fee dcrutil.Amount) (*lnwire.ClosingSigned, error) {
	...
	chancloserLog.Infof("ChannelPoint(%v): proposing fee of %v atoms to close "+
...
// our offered fee.
func ratchetFee(fee dcrutil.Amount, up bool) dcrutil.Amount {
...
func calcCompromiseFee(chanPoint wire.OutPoint, ourIdealFee, lastSentFee,
	remoteFee dcrutil.Amount) dcrutil.Amount {

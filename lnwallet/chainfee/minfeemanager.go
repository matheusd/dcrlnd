...
type fetchFee func() (AtomPerKByte, error)
...
func newMinFeeManager(minUpdateInterval time.Duration,
	...
	// floor.
	if minFee < FeePerKBFloor {
		minFee = FeePerKBFloor
	}
...
// our fee floor.
func (m *minFeeManager) fetchMinFee() AtomPerKByte {
	m.mu.Lock()
	defer m.mu.Unlock()
	...
	if m.minFeePerKW < FeePerKBFloor {
		m.minFeePerKW = FeePerKBFloor
	}

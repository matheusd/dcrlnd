...
// EPubKey is an Encoder for *secp256k1.PublicKey values. An error is returned if
// val is not a **secp256k1.PublicKey.
func EPubKey(w io.Writer, val interface{}, _ *[8]byte) error {
	if pk, ok := val.(**secp256k1.PublicKey); ok {
		_, err := w.Write((*pk).SerializeCompressed())
		return err
	}
	return NewTypeForEncodingErr(val, "*secp256k1.PublicKey")
}

// DPubKey is a Decoder for *secp256k1.PublicKey values. An error is returned if val
// is not a **secp256k1.PublicKey.
func DPubKey(r io.Reader, val interface{}, _ *[8]byte, l uint64) error {
	if pk, ok := val.(**secp256k1.PublicKey); ok && l == 33 {
		var b [33]byte
		_, err := io.ReadFull(r, b[:])
		if err != nil {
			return err
	...
	return NewTypeForDecodingErr(val, "*secp256k1.PublicKey", l, 33)
}
